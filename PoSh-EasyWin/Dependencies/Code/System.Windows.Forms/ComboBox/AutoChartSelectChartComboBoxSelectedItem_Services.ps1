...
foreach ($CollectionDir in $script:ListOfCollectedDataDirectories) {
    ...
    foreach ($CSVFile in $CSVFiles) { if ($CSVFile -match 'Services') { $script:AutoChart01CSVFileMatch += $CSVFile } }
} 
...
$AutoChartSelectFileButton.Add_Click({
    ...
    Generate-AutoChart01 -Filter 'Running'
        $script:AutoChart01FilterComboBox.SelectedIndex = 0

    Generate-AutoChart02 -Filter 'Running'
        $script:AutoChart02FilterComboBox.SelectedIndex = 0

    Generate-AutoChart03 -Filter 'Auto'
        $script:AutoChart03FilterComboBox.Text = 'Auto'

    Generate-AutoChart04 -Filter 'LocalSystem'
        $script:AutoChart04FilterComboBox.Text = 'LocalSystem'
...
$script:AutoChart01.Series.Add("Running Services")  
$script:AutoChart01.Series["Running Services"].Enabled           = $True
$script:AutoChart01.Series["Running Services"].BorderWidth       = 1
$script:AutoChart01.Series["Running Services"].IsVisibleInLegend = $false
$script:AutoChart01.Series["Running Services"].Chartarea         = 'Chart Area'
$script:AutoChart01.Series["Running Services"].Legend            = 'Legend'
$script:AutoChart01.Series["Running Services"].Font              = New-Object System.Drawing.Font @('Microsoft Sans Serif','9', [System.Drawing.FontStyle]::Normal)
$script:AutoChart01.Series["Running Services"]['PieLineColor']   = 'Black'
$script:AutoChart01.Series["Running Services"]['PieLabelStyle']  = 'Outside'
$script:AutoChart01.Series["Running Services"].ChartType         = 'Column'
$script:AutoChart01.Series["Running Services"].Color             = 'Red'
        ...
        function Generate-AutoChart01 ($Filter) {
            ...
            $script:AutoChart01UniqueDataFields = $script:AutoChartDataSource | Where-Object {$_.State -eq "$Filter" -or $_.Status -eq "$Filter"} | Select-Object -Property 'Name' | Sort-Object -Property 'Name' -Unique
            ...
            if ($script:AutoChart01UniqueDataFields.count -gt 0){
                ...
                $script:AutoChart01Title.Text = "$Filter Services"

                # If the Second field/Y Axis equals PSComputername, it counts it
                $script:AutoChart01OverallDataResults = @()

                $FilteredData = $script:AutoChartDataSource | Where-Object {$_.State -eq "$Filter" -or $_.Status -eq "$Filter"}
...
$script:AutoChart01FilterComboBox = New-Object System.Windows.Forms.ComboBox -Property @{
    Text      = 'Running' 
    Location  = @{ X = $script:AutoChart01OptionsButton.Location.X + 1 
                    Y = $script:AutoChart01OptionsButton.Location.Y - $script:AutoChart01OptionsButton.Size.Height - 5 }
    Size      = @{ Width  = 76
                    Height = 20 }     
    Font      = New-Object System.Drawing.Font("$Font",11,0,0,0)
...
$script:AutoChart01FilterComboBox.add_SelectedIndexChanged({
    Generate-AutoChart01 -Filter $script:AutoChart01FilterComboBox.SelectedItem
#    $script:AutoChart01.Series["Running Services"].Points.Clear()
#    $script:AutoChart01OverallDataResults | Sort-Object -Property UniqueCount | Select-Object -skip $script:AutoChart01TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart01TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart01.Series["Running Services"].Points.AddXY($_.DataField.Name,$_.UniqueCount)}
})
$script:AutoChart01FilterAvailable = @('Running','Stopped')
ForEach ($Item in $script:AutoChart01FilterAvailable) { $script:AutoChart01FilterComboBox.Items.Add($Item) }
$script:AutoChartsIndividualTab01.Controls.Add($script:AutoChart01FilterComboBox)
...
$script:AutoChart01ChartTypeComboBox.add_SelectedIndexChanged({
    $script:AutoChart01.Series["Running Services"].ChartType = $script:AutoChart01ChartTypeComboBox.SelectedItem
#    $script:AutoChart01.Series["Running Services"].Points.Clear()
#    $script:AutoChart01OverallDataResults | Sort-Object -Property UniqueCount | Select-Object -skip $script:AutoChart01TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart01TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart01.Series["Running Services"].Points.AddXY($_.DataField.Name,$_.UniqueCount)}
})
...
$script:AutoChart01ChangeColorComboBox.add_SelectedIndexChanged({
    $script:AutoChart01.Series["Running Services"].Color = $script:AutoChart01ChangeColorComboBox.SelectedItem
})
...
$script:AutoChart01CheckDiffButton.Add_Click({
    ...
    $script:AutoChart01InvestDiffDropDownComboBox.Add_KeyDown({ if ($_.KeyCode -eq "Enter") { script:InvestigateDifference-AutoChart01  -Filter $script:AutoChart01FilterComboBox.SelectedItem }})
    $script:AutoChart01InvestDiffDropDownComboBox.Add_Click({ script:InvestigateDifference-AutoChart01 -Filter $script:AutoChart01FilterComboBox.SelectedItem })
    ...
    $script:AutoChart01InvestDiffExecuteButton.Add_KeyDown({ if ($_.KeyCode -eq "Enter") { script:InvestigateDifference-AutoChart01 -Filter $script:AutoChart01FilterComboBox.SelectedItem }})
    $script:AutoChart01InvestDiffExecuteButton.Add_Click({ script:InvestigateDifference-AutoChart01 -Filter $script:AutoChart01FilterComboBox.SelectedItem })
...
$script:AutoChart01ExpandChartButton = New-Object System.Windows.Forms.Button -Property @{
    Text   = 'Multi-Series'
...
CommonButtonSettings -Button $script:AutoChart01ExpandChartButton
$script:AutoChart01ManipulationPanel.Controls.Add($script:AutoChart01ExpandChartButton)
...
$script:AutoChart01Results.Add_Click({ $script:AutoChartDataSource | Out-GridView -Title "$script:AutoChartCSVFileMostRecentCollection" }) 
...
### Auto Create Charts Data Series Recent
$script:AutoChart02.Series.Add("Running Services Per Host")  
$script:AutoChart02.Series["Running Services Per Host"].Enabled           = $True
$script:AutoChart02.Series["Running Services Per Host"].BorderWidth       = 1
$script:AutoChart02.Series["Running Services Per Host"].IsVisibleInLegend = $false
$script:AutoChart02.Series["Running Services Per Host"].Chartarea         = 'Chart Area'
$script:AutoChart02.Series["Running Services Per Host"].Legend            = 'Legend'
$script:AutoChart02.Series["Running Services Per Host"].Font              = New-Object System.Drawing.Font @('Microsoft Sans Serif','9', [System.Drawing.FontStyle]::Normal)
$script:AutoChart02.Series["Running Services Per Host"]['PieLineColor']   = 'Black'
$script:AutoChart02.Series["Running Services Per Host"]['PieLabelStyle']  = 'Outside'
$script:AutoChart02.Series["Running Services Per Host"].ChartType         = 'DoughNut'
$script:AutoChart02.Series["Running Services Per Host"].Color             = 'Blue'
        
        function Generate-AutoChart02 ($Filter) {
            $script:AutoChart02CsvFileHosts     = $script:AutoChartDataSource | Select-Object -Property 'PSComputerName' -ExpandProperty 'PSComputerName' | Sort-Object -Unique
            $script:AutoChart02UniqueDataFields = $script:AutoChartDataSource | Where-Object {$_.State -eq "$Filter" -or $_.Status -eq "$Filter"} | Select-Object -Property 'Name' -ExpandProperty 'Name' | Sort-Object  -Unique
            ...
            if ($script:AutoChart02UniqueDataFields.count -gt 0){
                ...
                $script:AutoChart02Title.Text = "$Filter Services Per Host"
                ...
                $FilteredData = $script:AutoChartDataSource | Where-Object {$_.State -eq "$Filter" -or $_.Status -eq "$Filter"} | Sort-Object PSComputerName
                ...
                $script:AutoChart02OverallDataResults += $AutoChart02YDataResults
                $script:AutoChart02OverallDataResults | Sort-Object -Property ResultsCount | ForEach-Object { $script:AutoChart02.Series["Running Services Per Host"].Points.AddXY($_.Computer,$_.ResultsCount) }

                $script:AutoChart02.Series["Running Services Per Host"].Points.Clear()
                $script:AutoChart02OverallDataResults | Sort-Object -Property ResultsCount | Select-Object -skip $script:AutoChart02TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart02TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart02.Series["Running Services Per Host"].Points.AddXY($_.Computer,$_.ResultsCount)}
...
# A filter combobox to modify what is displayed
$script:AutoChart02FilterComboBox = New-Object System.Windows.Forms.ComboBox -Property @{
    Text      = 'Running' 
    Location  = @{ X = $script:AutoChart02OptionsButton.Location.X + 1
                    Y = $script:AutoChart02OptionsButton.Location.Y - $script:AutoChart02OptionsButton.Size.Height - 5 }
    Size      = @{ Width  = 76
                    Height = 20 }     
    Font      = New-Object System.Drawing.Font("$Font",11,0,0,0)
    AutoCompleteSource = "ListItems"
...
$script:AutoChart02FilterComboBox.add_SelectedIndexChanged({
    Generate-AutoChart02 -Filter $script:AutoChart02FilterComboBox.SelectedItem
#    $script:AutoChart02.Series["Running Services Per Host"].Points.Clear()
#    $script:AutoChart02OverallDataResults | Sort-Object -Property ResultsCount | Select-Object -skip $script:AutoChart02TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart02TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart02.Series["Running Services Per Host"].Points.AddXY($_.Computer,$_.ResultsCount)}
})
$script:AutoChart02FilterAvailable = @('Running','Stopped')
ForEach ($Item in $script:AutoChart02FilterAvailable) { $script:AutoChart02FilterComboBox.Items.Add($Item) }
$script:AutoChartsIndividualTab01.Controls.Add($script:AutoChart02FilterComboBox)
...
$script:AutoChart02ChartTypeComboBox.add_SelectedIndexChanged({
    $script:AutoChart02.Series["Running Services Per Host"].ChartType = $script:AutoChart02ChartTypeComboBox.SelectedItem
#    $script:AutoChart02.Series["Running Services Per Host"].Points.Clear()
#    $script:AutoChart02OverallDataResults | Sort-Object -Property ResultsCount | Select-Object -skip $script:AutoChart02TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart02TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart02.Series["Running Services Per Host"].Points.AddXY($_.Computer,$_.ResultsCount)}
})
...
$script:AutoChart02ChangeColorComboBox.add_SelectedIndexChanged({
    $script:AutoChart02.Series["Running Services Per Host"].Color = $script:AutoChart02ChangeColorComboBox.SelectedItem
})
...
$script:AutoChart02ExpandChartButton = New-Object System.Windows.Forms.Button -Property @{
    Text   = 'Multi-Series'
...
CommonButtonSettings -Button $script:AutoChart02ExpandChartButton
$script:AutoChart02ManipulationPanel.Controls.Add($script:AutoChart02ExpandChartButton)
...
$script:AutoChart02Results.Add_Click({ $script:AutoChartDataSource | Out-GridView -Title "$script:AutoChartCSVFileMostRecentCollection" }) 
...
### Auto Create Charts Data Series Recent
$script:AutoChart03.Series.Add("Automatic Startup Services")  
$script:AutoChart03.Series["Automatic Startup Services"].Enabled           = $True
$script:AutoChart03.Series["Automatic Startup Services"].BorderWidth       = 1
$script:AutoChart03.Series["Automatic Startup Services"].IsVisibleInLegend = $false
$script:AutoChart03.Series["Automatic Startup Services"].Chartarea         = 'Chart Area'
$script:AutoChart03.Series["Automatic Startup Services"].Legend            = 'Legend'
$script:AutoChart03.Series["Automatic Startup Services"].Font              = New-Object System.Drawing.Font @('Microsoft Sans Serif','9', [System.Drawing.FontStyle]::Normal)
$script:AutoChart03.Series["Automatic Startup Services"]['PieLineColor']   = 'Black'
$script:AutoChart03.Series["Automatic Startup Services"]['PieLabelStyle']  = 'Outside'
$script:AutoChart03.Series["Automatic Startup Services"].ChartType         = 'Column'
$script:AutoChart03.Series["Automatic Startup Services"].Color             = 'Green'
        ...
        function Generate-AutoChart03 ($Filter) { 
            ...
            $script:AutoChart03UniqueDataFields  = $script:AutoChartDataSource | Where-Object {$_.StartType -match $Filter -or $_.StartMode -match $Filter} | Select-Object -Property 'Name' | Sort-Object -Property 'Name' -Unique
            ...
            if ($script:AutoChart03UniqueDataFields.count -gt 0){
                ...
                $script:AutoChart03Title.Text = "$Filter Startup Services"
                
                # If the Second field/Y Axis equals PSComputername, it counts it
                $script:AutoChart03OverallDataResults = @()

                $FilteredData = $script:AutoChartDataSource | Where-Object {$_.StartType -match $Filter -or $_.StartMode -match $Filter}
...
#    $script:AutoChart03.Series["Automatic Startup Services"].Points.Clear()
#    $script:AutoChart03OverallDataResults | Sort-Object -Property UniqueCount | Select-Object -skip $script:AutoChart03TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart03TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart03.Series["Automatic Startup Services"].Points.AddXY($_.DataField.Name,$_.UniqueCount)}
})
$script:AutoChart03FilterAvailable = @()
$script:AutoChartDataSource | Select-Object StartMode -Unique | ForEach-Object { if ($script:AutoChart03FilterAvailable -inotcontains $_.StartMode) {$script:AutoChart03FilterAvailable += $_.StartMode}}
if ($script:AutoChart03FilterAvailable.Count -eq 0){ $script:AutoChartDataSource | Select-Object StartType -Unique | ForEach-Object { if ($script:AutoChart03FilterAvailable -inotcontains $_.StartType) {$script:AutoChart03FilterAvailable += $_.StartType}} }
...
$script:AutoChart03ChartTypeComboBox.add_SelectedIndexChanged({
    $script:AutoChart03.Series["Automatic Startup Services"].ChartType = $script:AutoChart03ChartTypeComboBox.SelectedItem
#    $script:AutoChart03.Series["Automatic Startup Services"].Points.Clear()
#    $script:AutoChart03OverallDataResults | Sort-Object -Property UniqueCount | Select-Object -skip $script:AutoChart03TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart03TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart03.Series["Automatic Startup Services"].Points.AddXY($_.DataField.Name,$_.UniqueCount)}
})
...
$script:AutoChart03ChangeColorComboBox.add_SelectedIndexChanged({
    $script:AutoChart03.Series["Automatic Startup Services"].Color = $script:AutoChart03ChangeColorComboBox.SelectedItem
})
...
$script:AutoChart03Results.Add_Click({ $script:AutoChartDataSource | Out-GridView -Title "$script:AutoChartCSVFileMostRecentCollection" }) 
...
### Auto Create Charts Data Series Recent
$script:AutoChart04.Series.Add("Services Started By LocalSystem")  
$script:AutoChart04.Series["Services Started By LocalSystem"].Enabled           = $True
$script:AutoChart04.Series["Services Started By LocalSystem"].BorderWidth       = 1
$script:AutoChart04.Series["Services Started By LocalSystem"].IsVisibleInLegend = $false
$script:AutoChart04.Series["Services Started By LocalSystem"].Chartarea         = 'Chart Area'
$script:AutoChart04.Series["Services Started By LocalSystem"].Legend            = 'Legend'
$script:AutoChart04.Series["Services Started By LocalSystem"].Font              = New-Object System.Drawing.Font @('Microsoft Sans Serif','9', [System.Drawing.FontStyle]::Normal)
$script:AutoChart04.Series["Services Started By LocalSystem"]['PieLineColor']   = 'Black'
$script:AutoChart04.Series["Services Started By LocalSystem"]['PieLabelStyle']  = 'Outside'
$script:AutoChart04.Series["Services Started By LocalSystem"].ChartType         = 'Column'
$script:AutoChart04.Series["Services Started By LocalSystem"].Color             = 'Orange'
        ...
        function Generate-AutoChart04 ($Filter) {
            ...
            $script:AutoChart04UniqueDataFields  = $script:AutoChartDataSource | Where-Object {$_.StartName -eq "$Filter"} | Select-Object -Property 'Name' | Sort-Object -Property 'Name' -Unique
            ...
            if ($script:AutoChart04UniqueDataFields.count -gt 0){
                ...
                $script:AutoChart04Title.Text = "Services Started By $Filter"

                # If the Second field/Y Axis equals PSComputername, it counts it
                $script:AutoChart04OverallDataResults = @()

                $FilteredData = $script:AutoChartDataSource | Where-Object {$_.StartName -eq "$Filter"}
...
#    $script:AutoChart04.Series["Services Started By LocalSystem"].Points.Clear()
#    $script:AutoChart04OverallDataResults | Sort-Object -Property UniqueCount | Select-Object -skip $script:AutoChart04TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart04TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart04.Series["Services Started By LocalSystem"].Points.AddXY($_.DataField.Name,$_.UniqueCount)}
})
$script:AutoChart04FilterAvailable = @()
$script:AutoChartDataSource | Select-Object StartName -Unique | ForEach-Object { if ($script:AutoChart04FilterAvailable -inotcontains $_.StartName) {$script:AutoChart04FilterAvailable += $_.StartName}}
...
$script:AutoChart04ChartTypeComboBox.add_SelectedIndexChanged({
    $script:AutoChart04.Series["Services Started By LocalSystem"].ChartType = $script:AutoChart04ChartTypeComboBox.SelectedItem
#    $script:AutoChart04.Series["Services Started By LocalSystem"].Points.Clear()
#    $script:AutoChart04OverallDataResults | Sort-Object -Property UniqueCount | Select-Object -skip $script:AutoChart04TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart04TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart04.Series["Services Started By LocalSystem"].Points.AddXY($_.DataField.Name,$_.UniqueCount)}
})
...
$script:AutoChart04ChangeColorComboBox.add_SelectedIndexChanged({
    $script:AutoChart04.Series["Services Started By LocalSystem"].Color = $script:AutoChart04ChangeColorComboBox.SelectedItem
})
...
$script:AutoChart04Results.Add_Click({ $script:AutoChartDataSource | Out-GridView -Title "$script:AutoChartCSVFileMostRecentCollection" }) 
...
### Auto Create Charts Data Series Recent
$script:AutoChart05.Series.Add("Processes That Started Services ")  
$script:AutoChart05.Series["Processes That Started Services "].Enabled           = $True
$script:AutoChart05.Series["Processes That Started Services "].BorderWidth       = 1
$script:AutoChart05.Series["Processes That Started Services "].IsVisibleInLegend = $false
$script:AutoChart05.Series["Processes That Started Services "].Chartarea         = 'Chart Area'
$script:AutoChart05.Series["Processes That Started Services "].Legend            = 'Legend'
$script:AutoChart05.Series["Processes That Started Services "].Font              = New-Object System.Drawing.Font @('Microsoft Sans Serif','9', [System.Drawing.FontStyle]::Normal)
$script:AutoChart05.Series["Processes That Started Services "]['PieLineColor']   = 'Black'
$script:AutoChart05.Series["Processes That Started Services "]['PieLabelStyle']  = 'Outside'
$script:AutoChart05.Series["Processes That Started Services "].ChartType         = 'Column'
$script:AutoChart05.Series["Processes That Started Services "].Color             = 'Brown'
        ...
        function Generate-AutoChart05 {
            ...
            $script:AutoChart05UniqueDataFields  = $script:AutoChartDataSource | Where-Object {$_.State -eq 'Running' -or $_.Status -eq 'Running'} | Select-Object -Property 'ProcessName' | Sort-Object -Property 'ProcessName' -Unique
            ...
            if ($script:AutoChart05UniqueDataFields.count -gt 0){
                ...
                $script:AutoChart05Title.Text = "Processes That Started Services "

                # If the Second field/Y Axis equals PSComputername, it counts it
                $script:AutoChart05OverallDataResults = @()

                $FilteredData = $script:AutoChartDataSource | Where-Object {$_.State -eq 'Running' -or $_.Status -eq 'Running'}
...
$script:AutoChart05ChartTypeComboBox.add_SelectedIndexChanged({
    $script:AutoChart05.Series["Processes That Started Services "].ChartType = $script:AutoChart05ChartTypeComboBox.SelectedItem
#    $script:AutoChart05.Series["Processes That Started Services "].Points.Clear()
#    $script:AutoChart05OverallDataResults | Sort-Object -Property UniqueCount | Select-Object -skip $script:AutoChart05TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart05TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart05.Series["Processes That Started Services "].Points.AddXY($_.DataField.ProcessName,$_.UniqueCount)}
})
...
$script:AutoChart05ChangeColorComboBox.add_SelectedIndexChanged({
    $script:AutoChart05.Series["Processes That Started Services "].Color = $script:AutoChart05ChangeColorComboBox.SelectedItem
})
...
function script:InvestigateDifference-AutoChart05 {    
    # List of Positive Endpoints that positively match
    $script:AutoChart05ImportCsvPosResults = $script:AutoChartDataSource | Where-Object 'ProcessName' -eq $($script:AutoChart05InvestDiffDropDownComboBox.Text) | Select-Object -ExpandProperty 'PSComputerName' -Unique
...
$script:AutoChart05CheckDiffButton.Add_Click({
    $script:AutoChart05InvestDiffDropDownArray = $script:AutoChartDataSource | Select-Object -Property 'ProcessName' -ExpandProperty 'ProcessName' | Sort-Object -Unique
...
$script:AutoChart05ExpandChartButton = New-Object System.Windows.Forms.Button -Property @{
    Text   = 'Multi-Series'
...
CommonButtonSettings -Button $script:AutoChart05ExpandChartButton
$script:AutoChart05ManipulationPanel.Controls.Add($script:AutoChart05ExpandChartButton)
...
CommonButtonSettings -Button $script:AutoChart05Results
$script:AutoChart05Results.Add_Click({ $script:AutoChartDataSource | Out-GridView -Title "$script:AutoChartCSVFileMostRecentCollection" }) 
...
### Auto Create Charts Data Series Recent
$script:AutoChart06.Series.Add("Accounts That Started Services")  
$script:AutoChart06.Series["Accounts That Started Services"].Enabled           = $True
$script:AutoChart06.Series["Accounts That Started Services"].BorderWidth       = 1
$script:AutoChart06.Series["Accounts That Started Services"].IsVisibleInLegend = $false
$script:AutoChart06.Series["Accounts That Started Services"].Chartarea         = 'Chart Area'
$script:AutoChart06.Series["Accounts That Started Services"].Legend            = 'Legend'
$script:AutoChart06.Series["Accounts That Started Services"].Font              = New-Object System.Drawing.Font @('Microsoft Sans Serif','9', [System.Drawing.FontStyle]::Normal)
$script:AutoChart06.Series["Accounts That Started Services"]['PieLineColor']   = 'Black'
$script:AutoChart06.Series["Accounts That Started Services"]['PieLabelStyle']  = 'Outside'
$script:AutoChart06.Series["Accounts That Started Services"].ChartType         = 'Column'
$script:AutoChart06.Series["Accounts That Started Services"].Color             = 'Gray'
        ...
        function Generate-AutoChart06 {
            ...
            $script:AutoChart06UniqueDataFields  = $script:AutoChartDataSource | Select-Object -Property 'StartName' | Sort-Object -Property 'StartName' -Unique
#            $script:AutoChart05UniqueDataFields  = $script:AutoChartDataSource | Where-Object {$_.State -eq 'Running' -or $_.Status -eq 'Running'} | Select-Object -Property 'ProcessName' | Sort-Object -Property 'ProcessName' -Unique
#batman
            ...
            if ($script:AutoChart06UniqueDataFields.count -gt 0){
                ...
                $script:AutoChart06Title.Text = "Accounts That Started Services"
...
$script:AutoChart06TrimOffLastGroupBox = New-Object System.Windows.Forms.GroupBox -Property @{
    Text        = "Trim Off Last: 0"
    Location    = @{ X = $script:AutoChart06TrimOffFirstGroupBox.Location.X + $script:AutoChart06TrimOffFirstGroupBox.Size.Width + 8
...
$script:AutoChart06ChartTypeComboBox.add_SelectedIndexChanged({
    $script:AutoChart06.Series["Accounts That Started Services"].ChartType = $script:AutoChart06ChartTypeComboBox.SelectedItem
#    $script:AutoChart06.Series["Accounts That Started Services"].Points.Clear()
#    $script:AutoChart06OverallDataResults | Sort-Object -Property UniqueCount | Select-Object -skip $script:AutoChart06TrimOffFirstTrackBarValue | Select-Object -SkipLast $script:AutoChart06TrimOffLastTrackBarValue | ForEach-Object {$script:AutoChart06.Series["Accounts That Started Services"].Points.AddXY($_.DataField.StartName,$_.UniqueCount)}
})
...
$script:AutoChart06ChangeColorComboBox.add_SelectedIndexChanged({
    $script:AutoChart06.Series["Accounts That Started Services"].Color = $script:AutoChart06ChangeColorComboBox.SelectedItem
})
...
function script:InvestigateDifference-AutoChart06 {    
    # List of Positive Endpoints that positively match
    $script:AutoChart06ImportCsvPosResults = $script:AutoChartDataSource | Where-Object 'StartName' -eq $($script:AutoChart06InvestDiffDropDownComboBox.Text) | Select-Object -ExpandProperty 'PSComputerName' -Unique
...
$script:AutoChart06CheckDiffButton.Add_Click({
    $script:AutoChart06InvestDiffDropDownArray = $script:AutoChartDataSource | Select-Object -Property 'StartName' -ExpandProperty 'StartName' | Sort-Object -Unique
...
$script:AutoChart06ExpandChartButton = New-Object System.Windows.Forms.Button -Property @{
    Text   = 'Multi-Series'
...
CommonButtonSettings -Button $script:AutoChart06ExpandChartButton
$script:AutoChart06ManipulationPanel.Controls.Add($script:AutoChart06ExpandChartButton)
...
CommonButtonSettings -Button $script:AutoChart06Results
$script:AutoChart06Results.Add_Click({ $script:AutoChartDataSource | Out-GridView -Title "$script:AutoChartCSVFileMostRecentCollection" }) 
