...
function Update-AutoChartsStartupCommands {
    ...
    Foreach ($TargetComputer in $ComputerNameList) {
        ...
        #>
            if ($ComputerListProvideCredentialsCheckBox.Checked) {
                if (!$script:Credential) { Create-NewCredentials }
                if ($AutoChartProtocolWinRMRadioButton.checked) {
                    $CollectionName = 'Startup Commands - (WinRM)'

                    Invoke-Command -ScriptBlock {Get-WmiObject Win32_StartupCommand} `
                    -ComputerName $TargetComputer `
                    -AsJob -JobName "PoSh-EasyWin: $($CollectionName) -- $($TargetComputer)" `
                    -Credential $script:Credential
                }
                elseif ($AutoChartProtocolRPCRadioButton.checked) {
                    $CollectionName = 'Startup Commands - (RPC)'

                    Start-Job -ScriptBlock {
                        param($TargetComputer,$script:Credential)
                        Get-WmiObject -Class Win32_StartupCommand -ComputerName $TargetComputer -Credential $script:Credential
                    } `
                ...
                elseif ($AutoChartProtocolSMBRadioButton.checked) {
                    $CollectionName = 'Startup Commands - (SMB)'
...
Elasped Time:  $($Timecount -replace '-','')"
                    ...
                    & $PsExecPath "\\$TargetComputer" -AcceptEULA -NoBanner powershell -command "Invoke-Command -ScriptBlock {Get-WmiObject Win32_StartupCommand} | ConvertTo-Csv -NoType" | ConvertFrom-Csv | Export-CSV "$($script:CollectionSavedDirectoryTextBox.text)\$CollectionName\$CollectionName -- $TargetComputer.csv" -NoTypeInformation
                    $script:ProgressBarFormProgressBar.Value += 1
                }
            }
        #}
