...
function Update-AutoChartsNetworkInterfaces {
        ...
        #>
            if ($ComputerListProvideCredentialsCheckBox.Checked) {
                ...
                if ($AutoChartProtocolWinRMRadioButton.checked) {
                    ...
                    Invoke-Command -ScriptBlock {Get-NetIPAddress | Select-Object *} `
                    -ComputerName $TargetComputer `
                    -AsJob -JobName "PoSh-EasyWin: $($CollectionName) -- $($TargetComputer)" `
                    -Credential $script:Credential
                }
                elseif ($AutoChartProtocolRPCRadioButton.checked) {
                    ...
                    Start-Job -ScriptBlock {
                        ...
                        Get-WmiObject -Class Win32_NetworkAdapterConfiguration -ComputerName $TargetComputer -Credential $script:Credential `
                        | Select-Object PSComputerName, @{n='InterfaceAlias';e={$_.Description}},  @{n='IPAddresss';e={$_.IPAddress[0]}}, @{n='AddressFamily';e={'IPv4'}}, @{n='PrefixOrigin';e={$_.DHCPEnabled}}, * -ErrorAction SilentlyContinue | Where-Object {$_.IPAddress -ne $null}
                    } `
...
Elasped Time:  $($Timecount -replace '-','')"
                    ...
                    & $PsExecPath "\\$TargetComputer" -AcceptEULA -NoBanner -u $UserName -p $Password powershell -command 'Invoke-Command -ScriptBlock {Get-NetIPAddress | ConvertTo-Csv -NoType' | ConvertFrom-Csv | Export-CSV "$($script:CollectionSavedDirectoryTextBox.text)\Results By Endpoints\$CollectionName\$CollectionName -- $TargetComputer.csv" -NoTypeInformation
