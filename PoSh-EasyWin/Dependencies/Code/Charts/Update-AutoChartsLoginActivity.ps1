...
function Update-AutoChartsLoginActivity {
    ...
    #$PullNewDataScriptPath = "$QueryCommandsAndScripts\Scripts-Host\Get-LoginActivityEnriched.ps1"
    ...
    Foreach ($TargetComputer in $ComputerNameList) {
        ...
        if ($AutoChartPullNewDataEnrichedCheckBox.checked) {
            if ($ComputerListProvideCredentialsCheckBox.Checked) {                        
                ...
                if ($AutoChartProtocolWinRMRadioButton.checked) {
                    $CollectionName = 'Get-LoginActivityEnriched - (WinRM) Script'
        ...
        #>
            if ($ComputerListProvideCredentialsCheckBox.Checked) {
                if (!$script:Credential) { Create-NewCredentials }
                if ($AutoChartProtocolWinRMRadioButton.checked) {
                    $CollectionName = 'Current Login Activity - (WinRM)'
                ...
                elseif ($AutoChartProtocolRPCRadioButton.checked) {
                    $CollectionName = 'Current Login Activity - (RPC)'

                    Start-Job -ScriptBlock {
                        param($TargetComputer,$script:Credential)
                        Get-WSManInstance -ResourceURI Shell -Enumerate -ComputerName $TargetComputer -Credential $script:Credential
                    } `
                ...
                elseif ($AutoChartProtocolSMBRadioButton.checked) {
                    $CollectionName = 'Current Login Activity - (SMB)'
...
Elasped Time:  $($Timecount -replace '-','')"
                    ...
                    & $PsExecPath "\\$TargetComputer" -AcceptEULA -NoBanner -u $UserName -p $Password powershell -command "Invoke-Command -ScriptBlock { Get-WSManInstance -ResourceURI Shell -Enumerate } | ConvertTo-Csv -NoType" | ConvertFrom-Csv | Export-CSV "$($script:CollectionSavedDirectoryTextBox.text)\Results By Endpoints\$CollectionName\$CollectionName -- $TargetComputer.csv" -NoTypeInformation
...
Elasped Time:  $($Timecount -replace '-','')"
                    ...
                    & $PsExecPath "\\$TargetComputer" -AcceptEULA -NoBanner powershell -command "Invoke-Command -ScriptBlock { Get-WSManInstance -ResourceURI Shell -Enumerate } | ConvertTo-Csv -NoType" | ConvertFrom-Csv | Export-CSV "$($script:CollectionSavedDirectoryTextBox.text)\$CollectionName\$CollectionName -- $TargetComputer.csv" -NoTypeInformation
                    $script:ProgressBarFormProgressBar.Value += 1
                }
            }
        #}
