...
function Invoke-DeepBlue24Hours {
...
function Create-Filter($file, $logname)
{
    ...
    if ($file -ne ""){
        switch ($logname){
            "Security"    {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);path=""$file"";ID=$sec_events}"}
            "System"      {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);path=""$file"";ID=$sys_events}"}
            "Application" {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);path=""$file"";ID=$app_events}"}
            "Applocker"   {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);path=""$file"";ID=$applocker_events}"}
            "Powershell"  {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);path=""$file"";ID=$powershell_events}"}
            "Sysmon"      {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);path=""$file"";ID=$sysmon_events}"}
            default       {"Logic error 1, should not reach here...";Exit 1}
        }
    }
    else{
        switch ($logname){
            "Security"    {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);Logname=""Security"";ID=$sec_events}"}
            "System"      {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);Logname=""System"";ID=$sys_events}"}
            "Application" {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);Logname=""Application"";ID=$app_events}"}
            "Applocker"   {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);logname=""Microsoft-Windows-AppLocker/EXE and DLL"";ID=$applocker_events}"}
            "Powershell"  {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);logname=""Microsoft-Windows-PowerShell/Operational"";ID=$powershell_events}"}
            "Sysmon"      {$filter="@{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);logname=""Microsoft-Windows-Sysmon/Operational"";ID=$sysmon_events}"}
