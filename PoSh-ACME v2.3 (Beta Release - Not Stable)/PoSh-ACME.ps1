...
$CommandsSelectionCheckedlistbox.Add_Click({
    ...
    If ($CommandsSelectionCheckedlistbox.SelectedItem -match 'Disk Info') {
        $MainListBox.Items.Clear()
        $CommandFile = Get-Content -Path "$CommandsHostDirectory\Disk Info.txt" -Force -ErrorAction SilentlyContinue | foreach {$_ + "`r`n"}
        foreach ($line in $CommandFile) {$MainListBox.Items.Add("$line")}
    }
...
$ExecuteScriptHandler= {
    ...
    # Runs commands if it passes the above checks
        ...
        # Runs the Commands
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Accounts - Local') {AccountsLocalCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'ARP Cache') {ARPCacheCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Startup Commands') {StartupCommandsCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'BIOS Info') {BIOSInfoCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Computer Info') {ComputerInfoCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Dates - Install, Bootup, System') {DatesCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Disk Info') {DiskInfoCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'DLLs Loaded by Processes') {DLLsLoadedByProcessesCommand}
        ...
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Groups - Local') {GroupLocalCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Logon Info') {LogonInfoCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Logon Sessions') {LogonSessionsCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Logon User Status') {LogonUserStatusCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Mapped Drives') {MappedDrivesCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Memory - Capacity Info') {MemoryCapacityInfoCommand}
        ...
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Network Statistics IPv4 All') {NetworkStatisticsIPv4Command}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Network Statistics IPv4 TCP') {NetworkStatisticsIPv4TCPCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Network Statistics IPv4 UDP') {NetworkStatisticsIPv4UDPCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Network Statistics IPv4 ICMP') {NetworkStatisticsIPv4ICMPCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Network Statistics IPv6 All') {NetworkStatisticsIPv6Command}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Network Statistics IPv6 TCP') {NetworkStatisticsIPv6TCPCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Network Statistics IPv6 UDP') {NetworkStatisticsIPv6UDPCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Network Statistics IPv6 ICMP') {NetworkStatisticsIPv6ICMPCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Plug and Play Devices') {PlugAndPlayCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Port Proxy Rules') {PortProxyRulesCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Prefetch Files') {PrefetchFilesCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Process Tree - Lineage') {ProcessTreeLineageCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Processes - Enhanced with Hashes') {ProcessesEnhancedCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Processes - Standard') {ProcessesStandardCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Processor - CPU Info') {ProcessorCPUInfoCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Remote Capability Check') {RemoteCapabilityCheckCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Scheduled Tasks - schtasks') {ScheduledTasksCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Screen Saver Info') {ScreenSaverInfoCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Security Patches') {SecurityPatchesCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Services') {ServicesCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Shares') {SharesCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'Software Installed') {SoftwareInstalledCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'System Info') {SystemInfoCommand}
        if ($CommandsSelectionCheckedlistbox.CheckedItems -match 'USB Controller Devices') {USBControllerDevicesCommand}        
        
        if ($FileSearchDirectoryListingCheckbox.Checked) {FileSearchDirectoryListingCommand}
        if ($FileSearchFileSearchCheckbox.Checked){FileSearchFileSearchCommand}
        ...
        if ($EventLogsEventIDsManualEntryCheckbox.Checked) {EventLogsEventCodeManualEntryCommand}
        if ($EventLogsEventIDsIndividualSelectionCheckbox.Checked) {EventLogsEventCodeIndividualSelectionCommand}
        if ($EventLogsQuickPickSelectionCheckbox.Checked) {
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Application Event Logs') {ApplicationEventLogsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Application Event Logs Errors') {ApplicationEventLogsErrorsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Security Event Logs') {SecurityEventLogsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'System Event Logs') {SystemEventLogsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'System Event Logs Errors') {SystemEventLogsErrorsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Account Lockout') {AccountLockoutCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Account Management') {AccountManagementCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Account Management Events - Other') {AccountManagementEventsOtherCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Application Generated') {ApplicationGeneratedCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Application Group Management') {ApplicationGroupManagementCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Authentication Policy Change') {AuthenticationPolicyChangeCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Authorization Policy Change') {AuthorizationPolicyChangeCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Audit Policy Change') {AuditPolicyChangeCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Central Access Policy Staging') {CentralAccessPolicyStagingCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Certification Services') {CertificationServicesCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Computer Account Management') {ComputerAccountManagementCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Detailed Directory Service Replication') {DetailedDirectoryServiceReplicationCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Detailed File Share') {DetailedFileShareCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Directory Service Access') {DirectoryServiceAccessCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Directory Service Changes') {DirectoryServiceChangesCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Directory Service Replication') {DirectoryServiceReplicationCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Distribution Group Management') {DistributionGroupManagementCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'DPAPI Activity') {DPAPIActivityCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Group Membership') {GroupMembershipCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'File Share') {FileShareCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'File System') {FileSystemCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Filtering Platform Connection') {FilteringPlatformConnectionCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Filtering Platform Packet Drop') {FilteringPlatformPacketDropCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Filtering Platform Policy Change') {FilteringPlatformPolicyChangeCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Handle Manipulation') {HandleManipulationCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'IPSec Driver') {IPSecDriverCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'IPSec Extended Mode') {IPSecExtendedModeCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'IPSec Main Mode') {IPSecMainModeCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'IPSec Quick Mode') {IPSecQuickModeCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Kerberos Authentication Service') {KerberosAuthenticationServiceCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Kerberos Service Ticket Operations') {KerberosServiceTicketOperationsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Kernel Object') {KernelObjectCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Logon and Logoff Events') {LogonLogoffEventsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Logon and Logoff Events - Other') {LogonLogoffEventsOtherCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'MPSSVC Rule Level Policy Change') {MPSSVCRuleLevelPolicyChangeCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Network Policy Server') {NetworkPolicyServerCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Other Events') {OtherEventsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Other Object Access Events') {OtherObjectAccessEventsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Other Policy Change Events') {OtherPolicyChangeEventsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Other System Events') {OtherSystemEventsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'PNP Activity') {PNPActivityCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Process Creation and Termination') {ProcessCreationTerminationCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Registry') {RegistryCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Removeable Storage') {RemoveableStorageCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'RPC Events') {RPCEventsCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'SAM') {SAMCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Security Group Management') {SecurityGroupManagementCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Security State Change') {SecurityStateChangeCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Security System Extension') {SecuritySystemExtensionCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Sensitive and NonSensitive Privilege Use') {SensitiveandNonSensitivePrivilegeUseCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'Special Logon') {SpecialLogonCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'System Integrity') {SystemIntegrityCommand}
            if ($EventLogsQuickPickSelectionCheckedlistbox.CheckedItems -match 'User and Device Claims') {UserDeviceClaimsCommand}
        }

        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Account Details and User Information') {ActiveDirectoryAccountDetailsAndUserInfoCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Account Logon and Passowrd Policy') {ActiveDirectoryAccountLogonAndPassowrdPolicyCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Account Contact Information') {ActiveDirectoryAccountContactInfoCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Account Email Addresses') {ActiveDirectoryAccountEmailAddressesCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Account Phone Numbers') {ActiveDirectoryAccountPhoneNumbersCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Computers') {ActiveDirectoryComputersCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Default Domain Password Policy') {ActiveDirectoryDefaultDomainPasswordPolicyCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Groups') {ActiveDirectoryGroupsCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Group Membership By Groups') {ActiveDirectoryGroupMembershipByGroupsCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Group Membership by Users') {ActiveDirectoryGroupMembershipByUsersCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'Active Directory - Groups Without Account Members') {ActiveDirectoryGroupsWithoutAccountMembersCommand}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'DNS All Records (Server 2008)') {DNSAllRecordsServer2008Command}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'DNS Root Hints (Server 2008)') {DomainDNSRootHintsServer2008Command}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'DNS Zones (Server 2008)') {DomainDNSZonesServer2008Command}
        if ($ActiveDirectorySelectionCheckedlistbox.CheckedItems -match 'DNS Statistics (Server 2008)') {DomainDNSStatisticsServer2008Command}
