...
# Check for User Created Comamnds, if the file doesn't exist it will create one as an example
if ( Test-Path -Path $CustomWinRMCommands ) { $script:ImportCustomCommands = Import-Csv -Path $CustomWinRMCommands }
    ...
    $CollectionName = 'Processes (User Created Example)'
    $ExampleCommand = New-Object PSObject -Property @{ 
        ...
        #Command_WinRM_Script = "Invoke-Command -FilePath '`$CommandsHostDirectoryScripts\Get-ProcessEnhanced.ps1'"
    ...
    $CollectionName = 'Services (User Created Example)'
...
$script:AllEndpointCommands = @()
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Name, Enabled, SID, LastLogon, AccountExpires, PasswordRequired, UserMayChangePassword, PasswordExpires, PasswordLastSet, PasswordChangeableDate, Description, FullName' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName,Name, SID, ObjectClass, PrincipalSource'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, DisplayName, pathToSignedReportingExe' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, State, IPAddress, LinkLayerAddress, InterfaceAlias, InterfaceIndex' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_WMI       = 'PSComputerName, SMBIOSBIOSVersion, Name, Manufacturer, SerialNumber, Version, Description, @{Name="ReleaseDate";Expression={$_.ConvertToDatetime($_.ReleaseDate)}}, InstallDate' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, DateTime' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        #NOTE: 'Get-WmiObject -Class Win32_MappedLogicalDisk' -- Returns the drive letters for drives, but won’t tell you which shares the drives are mapped to. Weird, but true... wrote a script.
        #Command_WinRM_Script = $null
        ...
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Free, Used, Description, Provider, Root' 
        Properties_WMI       = 'PSComputerName, FreeSpace, Size, Description, ProviderName, DeviceID, @{name="DriveType";expression={ if ($_.DriveType -eq 2){"2: Removeable Disk"} elseif ($_.DriveType -eq 3){"3: Fixed Local Disk"} elseif ($_.DriveType -eq 4){"4: Network Disk"} elseif ($_.DriveType -eq 5){"5: Compact Disk"} else {"$($_.DriveType): Unknown"} }}, VolumeName' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Number, FriendlyName, Model, Manufacturer, SerialNumber, OperationalStatus, TotalSize, BootFromDisk, IsSystem, IsOfline, IsReadOnly, PartitionStyle, NumberOfPartitions, LargestFreeExtent, Location, UniqueId' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Entry, RecordName, RecordType, Status, Section, TimeToLive, Data' 
        Properties_WMI       = 'PSComputerName, *'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-DriverDetails.ps1'"
        ...
        Properties_PoSh      = 'PSComputerName, Online, ClassName, Driver, OriginalFileName, ClassDescription, BootCritical, DriverSignature, ProviderName, Date, Version, InBox, LogPath, LogLevel' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, PSDrive, Name, Value' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, MaximumKilobytes, MinimumRetentionDays, OverflowAction, @{name="Entries"; expression={($_.Entries).count}}, Log, EnableRaisingEvents' 
        Properties_WMI       = 'PSComputerName, *'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, IsEnabled, LogMode, MaximumSizeInBytes, RecordCount, LogName, LogType, LogIsolation, FileSize, IsLogFull, LastWriteTime, OldestRecordNumber'
        Properties_WMI       = 'PSComputerName, *'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Command_RPC_PoSh     = $null
        Command_WMI          = $null
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-FailedLogins.ps1'"
        Command_WinRM_PoSh   = $null
        Command_WinRM_WMI    = $null
        Command_WinRM_CMD    = $null
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, SourceComputer, Username, IPAddress, Date'
        Properties_WMI       = 'PSComputerName, *'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Enabled, Name, DisplayName, Description, DisplayGroup, Group, Profile, Direction, Action, EdgeTraversalPolicy, LooseSourceMapping, LocalOnlyMapping, Owner, EnforcementStatus, Status, PrimaryStatus, PolicyStoreSource, PolicyStoreSourceType' 
        Properties_WMI       = 'PSComputerName, *'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Name, Enabled, DefaultInboundAction, DefaultOutboundAction, AllowInboundRules, AllowLocalFirewallRules, AllowLocalIPsecRules, AllowUserApps, AllowUserPorts, AllowUnicastResponseToMulticast, NotifyOnListen, EnableStealthModeForIPsec, LogFileName, LogMaxsizeKilobytes, LogAllowed, LogBlocked, LogIgnored, DisabledInterfaceAliases' 
        Properties_WMI       = 'PSComputerName, *'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-NetworkConnectionsTCPEnhanced.ps1'"
        ...
        Properties_PoSh      = 'PSComputerName, State, LocalAddress, LocalPort, RemoteAddress, RemotePort, OwningProcess, CreationTime, OffloadState, AppliedSetting' 
        Properties_WMI       = 'PSComputerName, *'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, LocalAddress, LocalPort' 
        Properties_WMI       = 'PSComputerName, *'
...
#       Command_WinRS_WMIC   = "winrs wmic nic list brief"
        ...
        Properties_PoSh      = 'PSComputerName, IPAddress, InterfaceIndex, InterfaceAlias, AddressFamily, Type, PrefixLength, PrefixOrigin, SuffixOrigin, AddressState, ValidLifetime, PreferredLifetime, SkipAsSource, PolicyStore' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Name, Directory, IsReadOnly, CreationTime, LastAccessTime, LastWriteTime, Mode, Attributes' 
        Properties_WMI       = 'PSComputerName, Directory, Name, Length, CreationTime, LastWriteTime, LastAccessTime, Attributes, Mode' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Command_RPC_PoSh     = $null
        Command_WMI          = $null
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-ProcessTree.ps1'"
        Command_WinRM_PoSh   = $null
        Command_WinRM_WMI    = $null
        Command_WinRM_CMD    = $null
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Level, ProcessId, ProcessNameIndented, ParentPID, ParentProcessName, CreationDate, WorkingSetSize'
        Properties_WMI       = 'PSComputerName, *'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = "winrs wmic process list brief"
        Command_WinRS_CMD    = "winrs tasklist /V /FO CSV"
        Properties_PoSh      = 'PSComputerName, Name, ID, Path, StartTime, @{name="WorkingSetSize";expression={$_.WS}} , Handle, HandleCount, @{Name="ThreadCount";expression={($_.Threads).count}}, Company, Product, Description'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, TaskName, State, Date, Actions, TaskPath, Description' 
        Properties_WMI       = 'PSComputerName, *'
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Status, Name, DisplayName, RequiredServices, StartType, CanPauseAndContinue, CanShutdown, CanStop, ServiceHandle' 
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-SoftwareFromRegistry.ps1'"
        Command_WinRM_PoSh   = $null
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-StartupCommandsFromRegistry.ps1'"
        Command_WinRM_PoSh   = $null
...
#       Command_WinRS_WMIC   = "winrs wmic os list brief"
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Command_RPC_PoSh     = $null
        Command_WMI          = $null
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Detect-VMWareHosts.ps1'"
        Command_WinRM_PoSh   = $null
        Command_WinRM_WMI    = $null
        Command_WinRM_CMD    = $null
        Command_WinRS_WMIC   = $null
    ...
    $script:AllEndpointCommands += [PSCustomObject]@{ 
        ...
        Command_RPC_PoSh     = $null
        Command_WMI          = $null
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-WinSxSCount.ps1'"
        Command_WinRM_PoSh   = $null
        Command_WinRM_WMI    = $null
        Command_WinRM_CMD    = $null
        Command_WinRS_WMIC   = $null
...
#==========================================================================================================================================
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Name, Enabled, LockedOut, AccountLockoutTime, LogonWorkStations, LastLogonDate, LastBadPasswordAttempt, PasswordLastSet, PasswordExpired, PasswordNeverExpires, PasswordNotRequired, CannotChangePassword'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Name, Title, Organization, Office, POBox, StreetAddress, City, State, PostalCode, Fax, OfficePhone, HomePhone, MobilePhone, EmailAddress'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Name, Enabled, LockedOut, LastLogonDate, AccountExpirationDate, PasswordExpired, PaswordNeverExpires, SID, SamAccountName, UserPrincipalName, DistinguishedName, ObjectClass, ObjectGUID'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Name, PasswordLastSet, Enabled, SamAccountName, SID'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Name, ObjectClass, WhenCreated, WhenChanged, Deleted, LastLogon, LogonCount, DistinguishedName, ObjectSID'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, SchemaMaster'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, DomainNamingMaster'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    #    Command_WinRM_CMD    = 'Invoke-Command -ScriptBlock { cmd c\ netdom query PDC }'
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, PDCEmulator'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, RIDMaster'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, InfrastructureMaster'
        Properties_WMI       = 'PSComputerName, *'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-ADDomainTrusts.ps1'"
        Command_WinRM_PoSh   = $null
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, TrustedDomain, TrustAttributes, TrustDirection, TrustType'
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Name, SID, SIDHistory, GroupCategory, GroupScope, @{n="Member"; e={($_.Members -replace "CN=","`nCN=") }}, Members, MemberOf, WhenCreated, WhenChanged, DistinguishedName, Description, ProtectedFromAccidentalDeletion'
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_RPC_PoSh     = $null
        Command_WMI          = $null
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-ADGroupMembershipByGroups.ps1'"
        Command_WinRM_PoSh   = $null
        Command_WinRM_WMI    = $null
        Command_WinRM_CMD    = $null
        Command_WinRS_WMIC   = $null
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_RPC_PoSh     = $null
        Command_WMI          = $null
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-ADGroupMembershipByUsers.ps1'"
        Command_WinRM_PoSh   = $null
        Command_WinRM_WMI    = $null
        Command_WinRM_CMD    = $null
        Command_WinRS_WMIC   = $null
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Command_WinRS_WMIC   = $null
        Command_WinRS_CMD    = $null
        Properties_PoSh      = 'PSComputerName, Name, ObjectClass, DistinguishedName, GroupCategory, GroupScope, ObjectGUID, SamAccountName, SID'
        Properties_WMI       = 'PSComputerName, *'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, LastClearTime, ServerStartTime, TimeElapsedSinceLastClearedStatistics, TimeElapsedSinceLastClearedStatisticsBetweenRestart, TimeElapsedSinceServerStart, TimeElapsedSinceServerStartBetweenRestart'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, TcpClientConnections, TcpQueries, TcpQueriesSent, TcpResponses, TcpResponsesReceived, UdpQueries, UdpQueriesSent, UdpResponses, UdpResponsesReceived'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Notify, Standard, TKeyNego, TotalQueries, TypeA, TypeAll, TypeAxfr, TypeIxfr, TypeMx, TypeNs, TypeOther, TypePtr, TypeSoa, TypeSrv, Update'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Enabled, List'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Enable, GlobalOverLocal, PreferAAAA, AlwaysQueryServer, EnableEDnsProbes, BlockUpdates, SendTimeout, ServerQueryInterval'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, UseRootHint, Timeout, EnableReordering, IPAddress, ReorderedIPAddress'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, NoRefreshInterval, RefreshInterval, ScavengingInterval, ScavengingState, LastScavengeTime'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, ComputerName, MajorVersion, MinorVersion, BuildNumber, IsReadOnlyDC, EnableDnsSec, EnableIPv6, ListeningIPAddress, AllIPAddress'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, Enable, AdditionalTimeout, RetryInterval, Timeout, SecureResponse'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, ZoneName, ZoneType, IsAutoCreated, IsDsIntegrated, IsReverseLookupZone, IsSigned'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        ...
        Properties_PoSh      = 'PSComputerName, ZoneName, AgingEnabled, AvailForScavengeTime, RefreshInterval, NoRefreshInterval, ScavengeServers'
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
    ...
    $script:AllActiveDirectoryCommands += [PSCustomObject]@{ 
        Name                 = $CollectionName
        Type                 = ""
        #Command_WinRM_Script = "Invoke-Command -FilePath '$CommandsHostDirectoryScripts\Get-ADOrganizationalUnitPermissions.ps1'"
        Message              = '_____'
        NotesPath            = "$CommandsHostDirectoryNotes\ADDS - $CollectionName.txt"
        ExportFileName       = "$CollectionName"
    }
...
function Initialize-CommandsTreeView {
    ...
    $script:TreeNodeImportCustomCommands = New-Object -TypeName System.Windows.Forms.TreeNode -ArgumentList "3) User Added Commands          " -Property @{
        Tag       = "Custom Commands"
        NodeFont  = New-Object System.Drawing.Font("$Font",11,1,2,1)
        ForeColor = [System.Drawing.Color]::FromArgb(0,0,0,0)
    }
...
Function View-CommandsTreeViewMethod {
    ...
    # Adds Imported Custom Command nodes
    Foreach ($Command in $script:ImportCustomCommands) {
        if ($Command.Command_RPC_PoSh) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $("{0,-13}{1}" -f "[RPC]", "PowerShell Cmdlets") -Entry "(RPC) PoSh -- $($Command.Name)" -ToolTip $Command.Command_RPC_PoSh }
        #if ($Command.Command_RPC_CMD) {Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $("{0,-13}{1}" -f "[RPC]", "Windows CMD") -Entry "(RPC) CMD -- $($Command.Name)" -ToolTip $Command.Command_RPC_CMD}
        if ($Command.Command_WMI) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $("{0,-13}{1}" -f "[RPC]", "Windows Management Instrumentation (WMI)") -Entry "(RPC) WMI -- $($Command.Name)" -ToolTip $Command.Command_WMI }
        #if ($Command.Command_WinRS_WMIC) {Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $("{0,-10}{1}" -f "[WinRM]", "WMIC") -Entry "(WinRM) WMIC -- $($Command.Name)" -ToolTip $Command.Command_WinRS_WMIC}
        #if ($Command.Command_WinRS_CMD) {Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $("{0,-10}{1}" -f "[WinRM]", "Windows CMD") -Entry "(WinRM) CMD -- $($Command.Name)" -ToolTip $Command.Command_WinRS_CMD}
        if ($Command.Command_WinRM_Script) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $("{0,-10}{1}" -f "[WinRM]", "PowerShell Scripts") -Entry "(WinRM) Script -- $($Command.Name)" -ToolTip $Command.Command_WinRM_Script }        
        if ($Command.Command_WinRM_PoSh) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $("{0,-10}{1}" -f "[WinRM]", "PowerShell Cmdlets") -Entry "(WinRM) PoSh -- $($Command.Name)" -ToolTip $Command.Command_WinRM_PoSh }
        if ($Command.Command_WinRM_WMI) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $("{0,-10}{1}" -f "[WinRM]", "Windows Management Instrumentation (WMI)") -Entry "(WinRM) WMI -- $($Command.Name)" -ToolTip $Command.Command_WinRM_WMI }
        #if ($Command.Command_WinRM_CMD) {Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $("{0,-10}{1}" -f "[WinRM]", "Windows CMD") -Entry "(WinRM) CMD -- $($Command.Name)" -ToolTip $Command.Command_WinRM_CMD}
    }
}
...
Function View-CommandsTreeViewQuery {
    ...
    # Adds Imported Custom Command nodes
    Foreach ($Command in $script:ImportCustomCommands) {
        if ($Command.Command_RPC_PoSh) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $Command.Name -Entry "(RPC) PoSh -- $($Command.Name)" -ToolTip $Command.Command_RPC_PoSh }
        #if ($Command.Command_RPC_CMD) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $Command.Name -Entry "(RPC) CMD -- $($Command.Name)" -ToolTip $Command.Command_RPC_CMD }
        if ($Command.Command_WMI) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $Command.Name -Entry "(RPC) WMI -- $($Command.Name)" -ToolTip $Command.Command_WMI }
        #if ($Command.Command_WinRS_WMIC) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $Command.Name -Entry "(WinRM) WMIC -- $($Command.Name)" -ToolTip $Command.Command_WinRS_WMIC }
        #if ($Command.Command_WinRS_CMD) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $Command.Name -Entry "(WinRM) CMD -- $($Command.Name)" -ToolTip $Command.Command_WinRS_CMD }
        if ($Command.Command_WinRM_Script) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $Command.Name -Entry "(WinRM) Script -- $($Command.Name)" -ToolTip $Command.Command_WinRM_Script }        
        if ($Command.Command_WinRM_PoSh) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $Command.Name -Entry "(WinRM) PoSh -- $($Command.Name)" -ToolTip $Command.Command_WinRM_PoSh }
        if ($Command.Command_WinRM_WMI) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $Command.Name -Entry "(WinRM) WMI -- $($Command.Name)" -ToolTip $Command.Command_WinRM_WMI }
        #if ($Command.Command_WinRM_CMD) { Add-CommandsNode -RootNode $script:TreeNodeImportCustomCommands -Category $Command.Name -Entry "(WinRM) CMD -- $($Command.Name)" -ToolTip $Command.Command_WinRM_CMD }
    }
}
...
⦿ Invoke-Command { Get-WmiObject -Class Win32_NTLogEvent }  -ComputerName <Hostname>`n`n
"@  })
    #---------------------------------------
...
⦿ Get-WmiObject -Class Win32_NTLogEvent -ComputerName <Hostname>`n`n
"@  })
...
"@
    ...
    $CollectedDataDirectorySearchLimitLabel = New-Object System.Windows.Forms.Label -Property @{
        Text     = "Previous Collection Search Limit"
    ...
    $RekallWinPmemReportedResourcesGroupBox = New-Object System.Windows.Forms.GroupBox -Property @{
        ...
        $RekallWinPmemCheckRemoteResourcesButton.add_click({
            ...
            if ($RekallWinPmemCheckRemoteResourcesButton.Text -eq "Querying Endpoint") {
                ...
                $script:ReportedRAMAmount = Get-WmiObject -Class Win32_ComputerSystem -ComputerName $ComputerListCheckedBoxesSelected | Select-Object -ExpandProperty TotalPhysicalMemory
                ...
                $ReportedDiskInfo = Get-WmiObject -Class Win32_LogicalDisk -ComputerName $ComputerListCheckedBoxesSelected | Where-Object DriveType -eq 3 | Select-Object -Property Size, FreeSpace
                $Message = "$((Get-Date).ToString('yyyy/MM/dd HH:mm:ss')) - Get-WmiObject -Class Win32_LogicalDisk -ComputerName $ComputerListCheckedBoxesSelected | Select-Object -Property Size, FreeSpace"
                $Message | Add-Content -Path $LogFile
                ...
                foreach ($count in $(10..10)) {
                    if ($count -in @(0,5,10)) {
                        $LoadPercentageAverage += Get-WmiObject Win32_Processor -ComputerName $ComputerListCheckedBoxesSelected | Select-Object -Property LoadPercentage
                        $Message = "$((Get-Date).ToString('yyyy/MM/dd HH:mm:ss')) - Get-WmiObject Win32_Processor -ComputerName $ComputerListCheckedBoxesSelected | Select-Object -Property LoadPercentage"
                        $Message | Add-Content -Path $LogFile
                    }
...
function RekallWinPmemMemoryCaptureCommand {
    ...
    $ResultsListBox.Items.Insert(1,"$(($CollectionCommandStartTime).ToString('yyyy/MM/dd HH:mm:ss')) $CollectionName - $ComputerListCheckedBoxesSelected")
    Conduct-PreCommandCheck -CollectedDataTimeStampDirectory $CollectedDataTimeStampDirectory `
                            -IndividualHostResults $IndividualHostResults -CollectionName $CollectionName `
                            -TargetComputer $ComputerListCheckedBoxesSelected
    Create-LogEntry -TargetComputer $ComputerListCheckedBoxesSelected -CollectionName $CollectionName -LogFile $LogFile
    Function RekallWinPmemMemoryCaptureData {
        ...
        $MemoryCaptureFile   = "MemoryCapture-$($ComputerListCheckedBoxesSelected).raw"
        ...
        $CopyWinPmemToTargetHost = "Copy-Item '$ExternalPrograms\$WinPmem' '\\$ComputerListCheckedBoxesSelected\C$\$TempPath' -Force"
        ...
        $CopyMemoryScriptToTargetHost = Copy-Item "$ExternalPrograms\$MemoryCaptureScript" "\\$ComputerListCheckedBoxesSelected\C$\$TempPath" -Force
        ...
        $ExecuteMemoryCaptureOnTargetHost = "Invoke-WmiMethod -Class Win32_Process -Name Create -ComputerName $ComputerListCheckedBoxesSelected -ArgumentList `"PowerShell -WindowStyle Hidden -Command C:\$TempPath\$MemoryCaptureScript`" | Out-Null"
        ...
        # Checks to see if the process is still running
        while ($true) {
            $CheckWinPmemProcessOnTargetHost = "Get-WmiObject -Class Win32_Process -ComputerName $ComputerListCheckedBoxesSelected | Where-Object ProcessName -eq WinPmem.exe"
            $CheckWinPmemProcessOnTargetHostStatus = Invoke-Expression $CheckWinPmemProcessOnTargetHost
            $Message1 = "$((Get-Date).ToString('yyyy/MM/dd HH:mm:ss')) - Checking Memory Capture Status"
            ...
            if ($CheckWinPmemProcessOnTargetHostStatus) {                     
                ...
                $CopyMemoryCaptureFromTargetHost = "Copy-Item '\\$ComputerListCheckedBoxesSelected\C$\$TempPath\$MemoryCaptureFile' '$CollectedDataTimeStampDirectory' -Force"
                ...
                Remove-Item "\\$ComputerListCheckedBoxesSelected\C$\$TempPath\$MemoryCaptureScript" -Force
                    
                # Removes WinPmem and Memory Capture filefrom the TargetHost
                Remove-Item "\\$ComputerListCheckedBoxesSelected\C$\$TempPath\$WinPmem" -Force
                Remove-Item "\\$ComputerListCheckedBoxesSelected\C$\$TempPath\$MemoryCaptureFile" -Force
    ...
    $CollectionCommandDiffTime1 = New-TimeSpan -Start $CollectionCommandStartTime -End $CollectionCommandEndTime1
    $ResultsListBox.Items.RemoveAt(1)
    $ResultsListBox.Items.Insert(1,"$(($CollectionCommandStartTime).ToString('yyyy/MM/dd HH:mm:ss')) [$CollectionCommandDiffTime1]  $CollectionName - $ComputerListCheckedBoxesSelected")
...
$RekallWinPmemMemoryCaptureButton.add_click({ 
    ...
    elseif ($ComputerListCheckedBoxesSelected.count -eq 0) {
        [system.media.systemsounds]::Exclamation.play()
        $StatusListBox.Items.Clear()
        $StatusListBox.Items.Add("Rekall WinPmem:  Error")
        $ResultsListBox.Items.Clear()
...
$EventViewerButton.Add_Click({
    ...
    if ($ComputerListCheckedBoxesSelected.count -eq 1) {
        ...
        Show-EventLog -ComputerName $ComputerListCheckedBoxesSelected
...
$ComputerListRDPButton.Add_Click({
    ...
    if ($ComputerListCheckedBoxesSelected.count -eq 1) {
        if ($ComputerListProvideCredentialsCheckBox.Checked) {
            ...
            #cmdkey /generic:TERMSRV/$ComputerListCheckedBoxesSelected /user:$Username /pass:$Password
            cmdkey /add:$ComputerListCheckedBoxesSelected /user:$Username /pass:$Password
            mstsc /v:$($ComputerListCheckedBoxesSelected):3389
            #Start-Sleep -Seconds 1
            #cmdkey /delete:$ComputerListCheckedBoxesSelected 
        }
        else {
            mstsc /v:$($ComputerListCheckedBoxesSelected):3389
        }
        $StatusListBox.Items.Clear()
        $StatusListBox.Items.Add("Remote Desktop:  $($ComputerListCheckedBoxesSelected)")
        $ResultsListBox.Items.Clear()
        $ResultsListBox.Items.Add("mstsc /v:$($ComputerListCheckedBoxesSelected):3389")
        $LogMessage = "$((Get-Date).ToString('yyyy/MM/dd HH:mm:ss'))  Remote Desktop (RDP): $($ComputerListCheckedBoxesSelected)"
        $LogMessage | Add-Content -Path $LogFile
    }
    elseif ($ComputerListCheckedBoxesSelected.count -eq 0) {
        [system.media.systemsounds]::Exclamation.play()
...
⦿ Compatiable with 'Provide Creds' Button if permitted within the network`n`n
"@ })
...
$ComputerListPSSessionButton.Add_Click({
    ...
    if ($ComputerListCheckedBoxesSelected.count -eq 1) {        
        ...
        if ($ComputerListProvideCredentialsCheckBox.Checked) {
            ...
            $Username = $script:Credential.UserName
            $Password = $script:Credential.GetNetworkCredential().Password
            $ResultsListBox.Items.Add("Enter-PSSession -ComputerName $ComputerListCheckedBoxesSelected -Credential $script:Credential")                        
            start-process powershell -ArgumentList "-noexit Enter-PSSession -ComputerName $ComputerListCheckedBoxesSelected -Credential `$(New-Object pscredential('$Username'`,`$('$Password' | ConvertTo-SecureString -AsPlainText -Force)))"
        }

        else {
            $ResultsListBox.Items.Add("Enter-PSSession -ComputerName $ComputerListCheckedBoxesSelected")
...
⦿ Compatiable with 'Provide Creds' Button`n`n
"@ })
...
$ComputerListPsExecButton.Add_Click({
    ...
    if ($ComputerListCheckedBoxesSelected.count -eq 1) {
        ...
        if ($script:Credential) { 
            $ResultsListBox.Items.Add("./PsExec.exe -accepteula \\$ComputerListCheckedBoxesSelected -u '<domain\username>' -p '<password>' cmd")
            #$ResultsListBox.Items.Add("./PsExec.exe -accepteula \\$ComputerListCheckedBoxesSelected -u $UserName -p $Password cmd")
            Start-Process PowerShell -WindowStyle Hidden -ArgumentList "Start-Process -WindowStyle Hidden '$PsExecPath' -ArgumentList '-accepteula \\$ComputerListCheckedBoxesSelected $UseCredential cmd'"        
        }
        else { 
            $ResultsListBox.Items.Add("./PsExec.exe -accepteula \\$ComputerListCheckedBoxesSelected cmd")
            Start-Process PowerShell -WindowStyle Hidden -ArgumentList "Start-Process -WindowStyle Hidden '$PsExecPath' -ArgumentList '-accepteula \\$ComputerListCheckedBoxesSelected cmd'"
        }
...
$ProvideCredsButton.Add_MouseHover({
    ToolTipFunction -Title "Use Credentials" -Icon "Info" -Message @"
...
$Column5DownPosition += $Column5DownPositionShift - 9
...
$ComputerListMoveButton.Add_Click({
    ...
    if ($ComputerListCheckedBoxesSelected.count -ge 1) {
        ...
        $ComputerListTreeViewPopupExecuteButton.Add_Click({ Move-ComputerListTreeViewSelected })
...
#        if ($CommandsTreeViewQueryAsCompiledRadioButton.checked -eq $true) { $QueryCommands += @{ $Command.Name = @{ Name = $Command.Name ; Command = $Command.Command ; Properties = $Command.Properties }} }
