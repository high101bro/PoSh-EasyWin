    ...
    [Security.Principal.WindowsBuiltInRole] "Administrator"))
{
    ...
    $verify = [Microsoft.VisualBasic.Interaction]::MsgBox(`
        "Attention Under-Privileged User!`n   $([System.Security.Principal.WindowsIdentity]::GetCurrent().Name)`n`nThe queries executed to remotely collect data require elevated credentials. You can either run this script with elevated privileges or provide them when promopted.",`
        ...
        foreach ($Category in $root.Nodes) { 
            ...
            foreach ($Entry in $Category.nodes) { 
                if ($Entry.isselected) { 
                    ...
                    $MainListBox.Items.Clear()
                    $MainListBox.Items.Add("- Highlight (not checkbox) one hostname/IP to RDP, PSSession, or PSExec")
                    $MainListBox.Items.Add("- Checkbox any number of Categories, hostnames, or IPs to run any number of queries")
                }
            }       
        }         
    }
...
$ComputerListRDPButton.add_click({
    if ($script:ComputerListTreeViewSelected.Length -gt 0) {
        $mstsc = mstsc /v:$($script:ComputerListTreeViewSelected):3389
        $mstsc

        $StatusListBox.Items.Clear()
        $StatusListBox.Items.Add("Remote Desktop:  $($script:ComputerListTreeViewSelected)")
        $MainListBox.Items.Clear()
        $MainListBox.Items.Add("mstsc /v:$($script:ComputerListTreeViewSelected):3389")
        ...
        $MainListBox.Items.Add("       Make sure to highlight (not checkbox) a hostname/IP")
...
$ComputerListExecuteButton.ForeColor = "Red"
### $ComputerListExecuteButton.add_Click($ExecuteScriptHandler) ### Is located lower in the script
$Section2ManageListTab.Controls.Add($ComputerListExecuteButton)
