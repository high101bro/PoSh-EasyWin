    ...
    $RekallWinPmemReportedResourcesGroupBox = New-Object System.Windows.Forms.GroupBox -Property @{
        ...
        $RekallWinPmemCheckRemoteResourcesButton.add_click({
            ...
            if ($RekallWinPmemCheckRemoteResourcesButton.Text -eq "Querying Endpoint") {
                ...
                $script:ReportedRAMAmount = Get-WmiObject -Class Win32_ComputerSystem -ComputerName $script:ComputerTreeNodeSelected | Select-Object -ExpandProperty TotalPhysicalMemory
                ...
                $ReportedDiskInfo = Get-WmiObject -Class Win32_LogicalDisk -ComputerName $script:ComputerTreeNodeSelected | Where-Object {$_.DriveType -eq 3 -and $_.DeviceID -eq 'C:'} | Select-Object -Property Size, FreeSpace
                $Message = "$((Get-Date).ToString('yyyy/MM/dd HH:mm:ss')) - Get-WmiObject -Class Win32_LogicalDisk -ComputerName $script:ComputerTreeNodeSelected | Select-Object -Property Size, FreeSpace"
                $Message | Add-Content -Path $LogFile
                ...
                foreach ($count in $(10..10)) {
                    if ($count -in @(0,5,10)) {
                        $LoadPercentageAverage += Get-WmiObject Win32_Processor -ComputerName $script:ComputerTreeNodeSelected | Select-Object -Property LoadPercentage
                        $Message = "$((Get-Date).ToString('yyyy/MM/dd HH:mm:ss')) - Get-WmiObject Win32_Processor -ComputerName $script:ComputerTreeNodeSelected | Select-Object -Property LoadPercentage"
                        $Message | Add-Content -Path $LogFile
                    }
...
function Conduct-RekallWinPmemMemoryCapture {
    ...
    $ResultsListBox.Items.Insert(1,"$(($CollectionCommandStartTime).ToString('yyyy/MM/dd HH:mm:ss')) $CollectionName - $script:ComputerTreeNodeSelected")
    Conduct-PreCommandCheck -CollectedDataTimeStampDirectory $CollectedDataTimeStampDirectory `
                            ...
                            -TargetComputer $script:ComputerTreeNodeSelected
    Create-LogEntry -TargetComputer $script:ComputerTreeNodeSelected -CollectionName $CollectionName -LogFile $LogFile
    Function RekallWinPmemMemoryCaptureData {
        ...
        $MemoryCaptureFile   = "MemoryCapture-$($script:ComputerTreeNodeSelected).raw"
        ...
        $CopyWinPmemToTargetHost = "Copy-Item '$ExternalPrograms\$WinPmem' '\\$script:ComputerTreeNodeSelected\C$\$TempPath' -Force"
        ...
        $CopyMemoryScriptToTargetHost = Copy-Item "$ExternalPrograms\$MemoryCaptureScript" "\\$script:ComputerTreeNodeSelected\C$\$TempPath" -Force
        ...
        $ExecuteMemoryCaptureOnTargetHost = "Invoke-WmiMethod -Class Win32_Process -Name Create -ComputerName $script:ComputerTreeNodeSelected -ArgumentList `"PowerShell -WindowStyle Hidden -Command C:\$TempPath\$MemoryCaptureScript`" | Out-Null"
        ...
        while ($true) {
            $CheckWinPmemProcessOnTargetHost = "Get-WmiObject -Class Win32_Process -ComputerName $script:ComputerTreeNodeSelected | Where-Object ProcessName -eq WinPmem.exe"
            ...
            if ($CheckWinPmemProcessOnTargetHostStatus) { Start-Sleep -Seconds 10 }
                ...
                $CopyMemoryCaptureFromTargetHost = "Copy-Item '\\$script:ComputerTreeNodeSelected\C$\$TempPath\$MemoryCaptureFile' '$CollectedDataTimeStampDirectory' -Force"
                ...
                Remove-Item "\\$script:ComputerTreeNodeSelected\C$\$TempPath\$MemoryCaptureScript" -Force
                    
                # Removes WinPmem and Memory Capture filefrom the TargetHost
                Remove-Item "\\$script:ComputerTreeNodeSelected\C$\$TempPath\$WinPmem" -Force
                Remove-Item "\\$script:ComputerTreeNodeSelected\C$\$TempPath\$MemoryCaptureFile" -Force
                break
            }
        }
    }
    ...
    $ResultsListBox.Items.Insert(1,"$(($CollectionCommandStartTime).ToString('yyyy/MM/dd HH:mm:ss')) [$CollectionCommandDiffTime1]  $CollectionName - $script:ComputerTreeNodeSelected")
