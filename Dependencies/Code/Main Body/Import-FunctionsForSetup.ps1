function Create-LogEntry {
    ...
    if ($NoTargetComputer) {
        Add-Content -Path $PewLogFile -Value "$((Get-Date).ToString('yyyy/MM/dd HH:mm:ss'))  $Message"
    }
    else {
        Add-Content -Path $PewLogFile -Value "$((Get-Date).ToString('yyyy/MM/dd HH:mm:ss'))  $TargetComputer`: $Message"
    }
}
...
You can also set or delete the value in the following file:
    '$PewSettings\Form Scaling Modifier.txt'
...
Use the trackbar below to scale PoSh-EasyWin's interface:"
    ...
    $ResolutionCheckOkayButton = New-Object System.Windows.Forms.Button -Property @{
        ...
        Add_Click = {
            ...
            $script:ResolutionCheckScalingTrackBar.Value / 10 | Out-File "$PewSettings\Form Scaling Modifier.txt"
    ...
    Capabilities include:
        Multiple Queries to multiple endpoints
            ...
            Easily update the Command TreeView
                Endpoints Agnostics
                    $CommandsAndScripts\Commands - Endpoint.csv
                    $CommandsAndScripts\Scripts-Host\<lots of scripts>
                Active Directory Specific
                    $CommandsAndScripts\Commands - Active Directory.csv
                    $CommandsAndScripts\Scripts-AD\<lots of scripts>
            WinRM Query Builder
        ...
        $UserNoticeAcknowledgementCheckOkayButton = New-Object System.Windows.Forms.Button -Property @{
            ...
            Add_Click = {
                ...
                if ($ProceedAfterAcknowledgement){
                    Get-Date | Set-Content "$PewSettings\User Notice And Acknowledgement.txt"
                    $UserNoticeAcknowledgementForm.Close()
                }
                else {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUElILKZy1kIE3L3+7ehO1cHBt
# KcigggM6MIIDNjCCAh6gAwIBAgIQeugH5LewQKBKT6dPXhQ7sDANBgkqhkiG9w0B
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUgMdr0ZdWfxiAAYc4tRRsYcCehLcwDQYJKoZI
# hvcNAQEBBQAEggEAs865U0gxeiDadJPm4NTrPmMjwyU78e7cm4GiIELm61oIveul
# ONfRS1EEVpI09HdzU966sxDwB3/aD98JQLoxHLIqyOU5PikkVrvk7/vunMHghJ7d
# YpgbcPNp3qPyc8Niqx7h4H9nnsspwsIKCG7UMhdkVpM+i2jWeS3qG5LAbsCmhSqt
# GUrGUtNW8/hwZxXCQFMIcu32ng2be+YvvDtBU3kE7z42lNaXhutaKrzb0tFG+iZv
# p6Xo884C6/GoZ34rxIj/197Nw28SpPwWj0R9mxO//ohQUbRNF1bWzukjnoDQlbie
# CacjSZCeyZlxm/Px1dGYdSnGZvU1tA8zA0jYrw==
