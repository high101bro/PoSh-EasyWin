...
$ConnectButton.Add_Click({    
    ...
    If ($username -match "^[A-Za-z0-9_!]*$") {   
        ...
        If ($Server -AND $Username) {        
            ...
            $Paragraph.Inlines.Add((New-ChatMessage -Message $message -ForeGround DarkOrange -Bold))
            $Paragraph.Inlines.Add((New-ChatMessage -Message ("ATTEMPTING TO CONNECT TO THE CHAT SERVER: {0}" -f $Server) -ForeGround DarkOrange -Bold))
        ...
        If ($Messagequeue.Count -gt 0) {
            ...
            Switch ($Message) {
                {$_.Startswith("~B")} {
                    ...
                    $Paragraph.Inlines.Add((New-ChatMessage -Message ("[{0}] " -f (Get-Date).ToLongTimeString()) -ForeGround Gray))
                    $Paragraph.Inlines.Add((New-ChatMessage -Message ("[$env:COMPUTERNAME] {0}: " -f $split[0]) -ForeGround Black -Bold))
                ...
                {$_.Startswith("~M")} {
                    ...
                    $split = $data -split ("{0}" -f "~~")
                    $Paragraph.Inlines.Add((New-ChatMessage -Message ("[{0}] " -f (Get-Date).ToLongTimeString()) -ForeGround Gray))
                    $Paragraph.Inlines.Add((New-ChatMessage -Message ("[$env:COMPUTERNAME] {0}: " -f $split[0]) -ForeGround Blue -Bold))
                ...
                {$_.StartsWith("~C")} {
                    #Connect
                    $Message = ("[$env:COMPUTERNAME] {0}: CONNECTED TO THE CHAT SERVER" -f $_.SubString(2))  
                    $Paragraph.Inlines.Add((New-ChatMessage -Message ("[{0}] " -f (Get-Date).ToLongTimeString()) -ForeGround Gray))
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU7b4P0sDmpvDqw1XipdKpAIMb
# UsGgggM6MIIDNjCCAh6gAwIBAgIQeugH5LewQKBKT6dPXhQ7sDANBgkqhkiG9w0B
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQU4eoy4V3NUiQU8GUuoGlwePlY3YMwDQYJKoZI
# hvcNAQEBBQAEggEAZZjczR8LAsfhzhrEQLjvhezHOYuVaj1y3eAx3X99IiueFBXe
# 92XG9Q80UY7qZ+bI/2q8/q7CdXFEkiloWhhxoDAd3s49F068nfnt+GRzT4UjQcSs
# 8AYYGMZg3Q2hBsVYPR0JAc85oazDG4kV3c3LWaUzzXbY+zAHYDkU2cj/Gm75piuu
# I87wKteWH/4bHJXgepXly7me2Hxw2e4T5N3d8QaLeQgxv/C1ML2ZYNkUCheoqhXj
# k+cm5ClTt5iamnYcrs7F+6rH0IviP0GhP7TdC4Tl1hRSavYAwmJQH5arQ17iZbOz
# WU1mNVIfjs6dsIzqP9GfHAgDyOqBPO8cWPePRA==
