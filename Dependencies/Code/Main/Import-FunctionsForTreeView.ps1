        ...
        Foreach($Computer in $script:ComputerTreeViewData) {
            ...
            if ($SaveScan){
                ...
                if ($Computer.Name -in $Endpoints) {
                    ...
                    $ComputerTreeNodeSaveDataTemp | Add-Member -MemberType NoteProperty -Name Notes                   -Value "$(Get-Date) -- Enumeration Scan: $($script:ScanConnectedPort -join ', ')`n$($Computer.Notes)" -Force
                }
...
Update-FormProgress "Add-TreeViewComputer"
...
Update-FormProgress "Rename-TreeViewComputer"
function Rename-TreeViewComputer {
    <#
        .Description
        #batman create Rename-AccountsTreeNodeSelected equivalent function/integration - this function name is already "in use", but broken
            ...
            foreach ($Category in $root.Nodes) {
                ...
                foreach ($Entry in $Category.nodes) {
                    if ($Entry.IsSelected) {
                        ...
                        $Section3HostDataNotesRichTextBox.Text = "$((Get-Date).ToString('dd/MM/yyyy HH:mm:ss')) -- Previous Endpoint Name: $script:PreviousEndpointName`r`n" + $($script:ComputerTreeViewData | Where-Object {$_.Name -eq $Entry.Text}).Notes
                        Save-TreeViewData -Endpoint
                        break
                    }
                }
...
Update-FormProgress "Add-TreeViewCommand"
function Add-TreeViewCommand {
    <#
        .Description
        Adds a treenode to the specified root node... a node within a category node
    #>
    param (
        $RootNode,
        $Category,
...
Update-FormProgress "Initialize-TreeViewCommand"
function Initialize-TreeViewCommand {
    <#
        .Description
...
Update-FormProgress "Search-TreeViewCommand"
function Search-TreeViewCommand {
    <#
        .Description
            ...
            Foreach($Command in $script:AllCommands) {
                if (($SearchFound -inotcontains $Command) -and (
                    ...
                    if ($Command.Command_WinRM_Script) {
                        Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(WinRM) Script -- $($Command.Name)" -ToolTip $Command.Command_WinRM_Script
                        $SearchFound += $Command
                    }
                    if ($Command.Command_WinRM_PoSh)   {
                        Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(WinRM) PoSh -- $($Command.Name)" -ToolTip $Command.Command_WinRM_PoSh
                        $SearchFound += $Command
                    }
                    if ($Command.Command_WinRM_WMI)    {
                        Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(WinRM) WMI -- $($Command.Name)" -ToolTip $Command.Command_WinRM_WMI
                        $SearchFound += $Command
                    }
                    #if ($Command.Command_WinRM_CMD)    {
                    #    Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(WinRM) CMD -- $($Command.Name)" -ToolTip $Command.Command_WinRM_CMD
                    #    $SearchFound += $Command
                    #}
                    #if ($Command.Command_RPC_PoSh)     {
                    #    Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(RPC) PoSh -- $($Command.Name)" -ToolTip $Command.Command_RPC_PoSh
                    #    $SearchFound += $Command
                    #}
                    if ($Command.Command_RPC_WMI)          {
                        Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(RPC) WMI -- $($Command.Name)" -ToolTip $Command.Command_RPC_WMI
                        $SearchFound += $Command
                    }
                    #if ($Command.Command_RPC_CMD)     {
                    #    Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(RPC) CMD -- $($Command.Name)" -ToolTip $Command.Command_RPC_CMD
                    #    $SearchFound += $Command
                    #}
                    if ($Command.Command_SMB_PoSh)    {
                        Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(SMB) PoSh -- $($Command.Name)" -ToolTip $Command.Command_SMB_PoSh
                        $SearchFound += $Command
                    }
                    if ($Command.Command_SMB_WMI)    {
                        Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(SMB) WMI -- $($Command.Name)" -ToolTip $Command.Command_SMB_WMI
                        $SearchFound += $Command
                    }
                    if ($Command.Command_SMB_CMD)    {
                        Add-TreeViewCommand -RootNode $script:TreeNodeCommandSearch -Category $($CommandSearchText) -Entry "(SMB) CMD -- $($Command.Name)" -ToolTip $Command.Command_SMB_CMD
                        $SearchFound += $Command
                    }
                }
            }
...
Update-FormProgress "Update-TreeViewCommand"
function Update-TreeViewCommand {
    <#
        .Description
...
Update-FormProgress "View-TreeViewCommandMethod"
...
Update-FormProgress "View-TreeViewCommandQuery"
