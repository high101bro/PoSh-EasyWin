Update-FormProgress "Invoke-PingSweep"
...
#    $script:ProgressBarQueriesProgressBar.Value   = 0
    ...
    foreach ($Computer in $PingList) {
        ...
        Write-LogEntry -LogFile $PewLogFile -NoTargetComputer -Message " - $ping"
...
Update-FormProgress "Invoke-PortScan"
function Invoke-PortScan {
    ...
    param (
        ...
        $FirstPort,
        $LastPort,
        $EnumerationPortScanPortQuickPickComboBoxSelectedItem
    )

    $InformationTabControl.SelectedTab = $Section3ResultsTab

    if ($EndpointList) {
        $IPsToScan = $ComputerList
    }
    else {
        $IPsToScan = @()
        ...
        $IPsToScan = $IPsToScan | Sort-Object { [System.Version]$_ } -Unique | Where-Object {$_ -ne ""}
    }
    ...
    if ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -eq "N/A") {
        $PortsToScan += ""
    }
    elseif ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -eq "Nmap Top 100 Ports") {
    ...
    elseif ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -eq "Nmap Top 1000 Ports") {
        Generate-PortsStatusMessage
    ...
    elseif ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -eq "Well-Known Ports (0-1023)") {
        Generate-PortsStatusMessage
        $PortsToScan += 0..1023
    }
    elseif ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -eq "Registered Ports (1024-49151)") {
        Generate-PortsStatusMessage
        $PortsToScan += 1024..49151
    }
    elseif ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -eq "Dynamic Ports (49152-65535)") {
        Generate-PortsStatusMessage
        $PortsToScan += 49152..65535
    }
    elseif ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -eq "All Ports (0-65535)") {
        Generate-PortsStatusMessage
        $PortsToScan += 0..65535
    }
    elseif ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -match "Previous Scan") {
        Generate-PortsStatusMessage
        $LastPortsScanned = $((Get-Content $PewLogFile | Select-String -Pattern "Ports To Be Scanned" | Select-Object -Last 1) -split '  ')[2]
    ...
    elseif ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -match "CustomPortsToScan") {
        Generate-PortsStatusMessage
    ...
    elseif ($EnumerationPortScanPortQuickPickComboBoxSelectedItem -eq "") { $PortsToScan += $null }
    ...
    Write-LogEntry -LogFile $PewLogFile -NoTargetComputer -Message "$TimeStamp  ==================== Port Scan Initiliaztion ===================="
    Write-LogEntry -LogFile $PewLogFile -NoTargetComputer -Message "$TimeStamp  Ports To Be Scanned:  $PortsToScan"
    $EnumerationComputerListBox.Items.Clear()
    #Removed For Testing#$ResultsListBox.Items.Clear()
    ...
    function PortScan {
        ...
        foreach ($Port in $PortsToScan) {
            ...
            if ($socket.Connected) {
                ...
                $socket.Dispose()
                $socket = $null
                Write-LogEntry -LogFile $PewLogFile -NoTargetComputer -Message "[Response Time: $contime ms] $IPAddress responded on port $Port"
                $ResultsListBox.Items.Insert(2,"$(Get-Date)  - [Response Time: $contime ms] $IPAddress responded on port $Port")
                Write-Host "$(Get-Date)  - [Response Time: $contime ms] $IPAddress responded on port $Port "
                $PortScanObjectResults += [PSCustomObject]@{
                    ScannedFrom     = $env:COMPUTERNAME
                    EndpointScanned = $IPAddress
                    Port            = $Port
                    ResponseTimeMS  = $contime
                    Responded       = $true
                    ScanTime        = Get-Date
                }
            }
        ...
        Save-TreeViewData -Endpoint -SaveScan -Ports $script:ScanConnectedPort -Endpoints $IPAddress
    ...
    foreach ($IPAddress in $IPsToScan) {
        if ($TestWithICMPFirst -eq $true) {
            ...
            if (Test-Connection -Count 1 -Quiet -ComputerName $IPAddress) {
                ...
                Write-LogEntry -LogFile $PewLogFile -NoTargetComputer -Message "ICMP Connection Test - $IPAddress is UP - Conducting Port Scan:)"
                PortScan -IPAddress $IPAddress
            }
            else {
                Write-LogEntry -LogFile $PewLogFile -NoTargetComputer -Message "ICMP Connection Test - $IPAddress is DOWN - No ports scanned)"
            }
        }
        elseif ($TestWithICMPFirst -eq $false) {
            $ResultsListBox.Items.Insert(1,"$(Get-Date)  Port Scan IP - $IPAddress")
            Write-LogEntry -LogFile $PewLogFile -NoTargetComputer -Message "Port Scan IP:  $IPAddress"
            PortScan -IPAddress $IPAddress
        }
        $script:ProgressBarQueriesProgressBar.Value += 1
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUu6EHGufPLFPCip+na+Cxr0ri
# WLagggM6MIIDNjCCAh6gAwIBAgIQeugH5LewQKBKT6dPXhQ7sDANBgkqhkiG9w0B
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUhW2o8QJK49M913JcAo0itpSeudswDQYJKoZI
# hvcNAQEBBQAEggEAUVGDsqUsaZ5pJlVM8Sf2OrXpk1P6IUlbOOYWrTscRP0defz4
# rctMbi1QeaFsnVN0a+ixaf2eHoqjDpv1lpxfosKezAIdl2OUtSXTjbaX3BWDFpi8
# AWDjhxcWokffrqZa7EP89oDm0Lw5OwK5R5Nh46OPWvxq4xID6BU8Id9pRSsMMjBe
# cRxegOkyfxV7iignOATm9nGAQA0nLZ8bz1K+G67Bn9vBP5Oopa3fCYw5GlTC1Vye
# /12IlKxSaKpY3lI5kEKwB7H3Vo6COKhLezg5MUg8YaAlkHxiZXz/81v4pWOguPTg
# +8gcYvmEt82I7/VMNC1IUi0AH89q3f4ww0sNEg==
