    ...
    if ($script:CommandTreeViewQueryMethodSelectionComboBox.SelectedItem -eq 'Monitor Jobs') {
        foreach ($TargetComputer in $script:ComputerList) {
            if ($script:ComputerListProvideCredentialsCheckBox.Checked) {
                ...
                Create-LogEntry -LogFile $LogFile -NoTargetComputer -Message "Invoke-Command -ScriptBlock `$EndpointDataSystemSnapshotScriptBlock -ArgumentList `$script:PSWriteHTMLSelectCommandsCheckedListBox.CheckedItems -ComputerName $TargetComputer -AsJob -JobName 'PoSh-EasyWin: $($CollectionName) -- $($TargetComputer)' -Credential `$script:Credential"
            }
            else {
                Invoke-Command -ScriptBlock $EndpointDataSystemSnapshotScriptBlock `
                -ComputerName $TargetComputer `
...
if ($PSWriteHTMLCheckedListBox.CheckedItems.Count -gt 0 -and `
    ...
    if ($script:PSWriteHTMLIndividualWebPagesCheckbox.checked -eq $false -and $script:CommandTreeViewQueryMethodSelectionComboBox.SelectedItem -eq 'Session Based') {
        New-HTML -TitleText 'PoSh-EasyWin' -FavIcon "$Dependencies\Images\favicon.jpg" -Online `
            ...
            if ($script:PSWriteHTMLFormOkay -eq $true -and $script:ComputerList.count -gt 0 -and $PSWriteHTMLCheckedListBox.CheckedItems -contains 'Endpoint Process Data') {
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUHmN3pymWFlqHfWZZRh4B1NUe
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUXkGDmFZrBWfiZdAtCH7ItrSYt58wDQYJKoZI
# hvcNAQEBBQAEggEAE8ltmDxzfaFTxccGQlTsZ7H6h6wxIDi6+pVn8SF39Jg4Yihr
# LiawZ5tjBNvjpHcUoy9vxwgd106yi7BTCKNgaMbaNYTWKKEo1ry21CBg0PUW3lWD
# iA9mecx7vsg618zqUb89Rr2ZVWAfjDpluIyZ2E5EtY7JN7mql+QITxDwrwTo9Rty
# d/hhyT3uTLZswKc2EqhihrRy3opivduCSFIRseEIZ0LJKF1ZgWBZSKODekFUv01G
# AZgaqx3nw/tBjREyw6OKFyVX3O5vn7LX/hbql8PzszZRZYOP6unGn7J30DBMopUI
# 1qzeDoliW9+LlbZ1XI5sM2BE1gWZGYLGIDFFYA==
