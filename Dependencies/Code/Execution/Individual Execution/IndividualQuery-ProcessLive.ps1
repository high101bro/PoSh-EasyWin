    ...
    function MonitorJobScriptBlock {
        ...
        foreach ($TargetComputer in $script:ComputerList) {
            ...
            $ProcessLiveScriptBlock = {
                function Query-ProcessInformation {
                    param(
                        $Name               = $null,
                        $Commandline        = $null,
                        $ParentName         = $null,
                    ...
                    if ($name -ne $null) {    
                        ...
                        foreach ($p in $name) {
                            if ($regex -eq $true) { 
                                $Processes += Get-WmiObject Win32_Process | Where {$_.Name -match $p}
                            }
                            elseif ($regex -eq $false) {  
                                $Processes += Get-WmiObject Win32_Process | Where {$_.Name -eq $p}
                            }
                        }
                    }
                    elseif ($Commandline -ne $null) {
                        $Processes = @()
                        foreach ($p in $Commandline) {
                            if ($regex -eq $true) { 
                                $Processes += Get-WmiObject Win32_Process | Where {$_.CommandLine -match $p}
                            }
                            elseif ($regex -eq $false) {  
                                $Processes += Get-WmiObject Win32_Process | Where {$_.CommandLine -eq $p}
                            }
                        }
                    }
                    else {
                        ...
                        if ($ParentName -ne $null) {
                            foreach ($p in $ParentName) {
                                ...
                                if ($regex -eq $true) {
                                    ...
                                    | Where-Object {$_.ParentProcessName -match $p}
                                }
                                elseif ($regex -eq $false) {
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.ParentProcessName -eq $p}
                                }
                            }
                        }
                        elseif ($OwnerSID -ne $null) {
                            foreach ($p in $OwnerSID) {
                                if ($regex -eq $true) {
                                    Write-Host "OwnerSID t = $p"
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    | Add-Member NoteProperty 'ParentProcessID'        $ParentProcessID   -PassThru -Force `
                                    | Add-Member NoteProperty 'ParentProcessName'      $ParentProcessName -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.Owner -match $p -or $_.OwnerSID -match $p}
                                }
                                elseif ($regex -eq $false) {
                                    Write-Host "OwnerSID f = $p"
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    | Add-Member NoteProperty 'ParentProcessID'        $ParentProcessID   -PassThru -Force `
                                    | Add-Member NoteProperty 'ParentProcessName'      $ParentProcessName -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.Owner -eq $p -or $_.OwnerSID -eq $p}
                                }
                            }
                        }
                        elseif ($ServiceInfo -ne $null) {
                            foreach ($p in $ServiceInfo) {
                                Write-Host $p
                                if ($regex -eq $true) {
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.ServiceInfo -match $p}
                                }
                                elseif ($regex -eq $false) {
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.ServiceInfo -eq $p}
                                }
                            }
                        }
                        elseif ($NetworkConnections -ne $null) {
                            foreach ($p in $NetworkConnections) {
                                Write-Host $p
                                if ($regex -eq $true) {
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.NetworkConnections -match $p}
                                }
                                elseif ($regex -eq $false) {
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.NetworkConnections -eq $p}
                                    write 'test'
                                }
                            }
                        }
                        elseif ($HashesSignerCerts -ne $null) {
                            foreach ($p in $HashesSignerCerts) {
                                Write-Host $p
                                if ($regex -eq $true) {
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.MD5Hash -match $p -or $_.SignerCertificate -match $p}
                                }
                                elseif ($regex -eq $false) {
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.MD5Hash -eq $p -or $_.SignerCertificate -eq $p}
                                }
                            }
                        }
                        elseif ($CompanyProduct -ne $null) {
                            foreach ($p in $CompanyProduct) {
                                Write-Host $p
                                if ($regex -eq $true) {
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.Company -match $p -or $_.Product -match $p -or $_.SignerCompany -match $p}
                                }
                                elseif ($regex -eq $false) {
                                    $EnrichedProcess `
                                    | Add-Member NoteProperty 'ProcessID'              $ProcessID         -PassThru -Force `
                                    ...
                                    | Add-Member NoteProperty 'Owner'                  $Owner             -PassThru -Force `
                                    | Add-Member NoteProperty 'OwnerSID'               $OwnerSID          -PassThru -Force `
                                    | Where-Object {$_.Company -eq $p -or $_.Product -eq $p -or $_.SignerCompany -eq $p}
                                }
                            }
                        }
                        else {
                    ...
                    $Processes | Foreach-Object { Write-ProcessTree -Process $PSItem} | Select-Object -Property Name, ProcessID, ParentProcessName, ParentProcessID, ServiceInfo,`
                ...
                #Query-ProcessInformation | ogv
                #Query-ProcessInformation -name @('lsass.exe','svchost.exe') -regex $false
                #Query-ProcessInformation -commandline @("C:\WINDOWS\system32\svchost.exe -k DcomLaunch -p -s LSM","C:\WINDOWS\System32\svchost.exe -k NetworkService -p -s LanmanWorkstation") -regex $false | ogv
                #Query-ProcessInformation -ParentName @("AsusDialService","nvcontainer","vmwp") -regex $false | ogv
                
                #Query-ProcessInformation -OwnerSID @('NT AUTHORITY\SYSTEM')  -regex $false | ogv
                Query-ProcessInformation -OwnerSID @('SYSTEM')  -regex $false | ogv
                
                #Query-ProcessInformation -OwnerSID @('S-1-5-19','S-1-5-18') |-regex $false  ogv
                #Query-ProcessInformation -ServiceInfo @('Spooler','Apple Mobile') -regex $false | ogv
                #Query-ProcessInformation -NetworkConnections @('Listen','192.168.68.148') -regex $false | ogv
                #Query-ProcessInformation -HashesSignerCerts @('634A0D','F7C2F2') -regex $false | ogv
                #Query-ProcessInformation -CompanyProduct @('Google LLC','Apple Inc.') -regex $false | ogv e
...
########################################
    ...
    $SearchString = ''
    foreach ($item in $ProcessLiveSearchParentName) {$SearchString += "$item`n" }
...
"@
    ...
    if ($ProcessLiveSearchNameRichTextBoxCheckbox) {
        Invoke-Command -ScriptBlock ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$ProcessLiveSearchNameRichTextBox,$null,$null,$null,$null,$null,$null,$null)
        Monitor-Jobs -CollectionName $CollectionName -MonitorMode -SMITH -SmithScript ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$ProcessLiveSearchNameRichTextBox,$null,$null,$null,$null,$null,$null,$null) -SmithFlag 'RetrieveFile' -InputValues $InputValues
    }


    if ($ProcessLiveSearchCommandlineCheckbox) {
        Invoke-Command -ScriptBlock ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$ProcessLiveSearchCommandline,$null,$null,$null,$null,$null,$null)
        Monitor-Jobs -CollectionName $CollectionName -MonitorMode -SMITH -SmithScript ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$ProcessLiveSearchCommandline,$null,$null,$null,$null,$null,$null) -SmithFlag 'RetrieveFile' -InputValues $InputValues
    }


    if ($ProcessLiveSearchParentNameCheckbox) {
        Invoke-Command -ScriptBlock ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$ProcessLiveSearchParentName,$null,$null,$null,$null,$null)
        Monitor-Jobs -CollectionName $CollectionName -MonitorMode -SMITH -SmithScript ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$ProcessLiveSearchParentName,$null,$null,$null,$null,$null) -SmithFlag 'RetrieveFile' -InputValues $InputValues
    }


    if ($ProcessLiveSearchOwnerSIDCheckbox) {
        Invoke-Command -ScriptBlock ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$ProcessLiveSearchOwnerSID,$null,$null,$null,$null)
        Monitor-Jobs -CollectionName $CollectionName -MonitorMode -SMITH -SmithScript ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$ProcessLiveSearchOwnerSID,$null,$null,$null,$null) -SmithFlag 'RetrieveFile' -InputValues $InputValues
    }


    if ($ProcessLiveSearchServiceInfoCheckbox) {
        Invoke-Command -ScriptBlock ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$null,$ProcessLiveSearchServiceInfo,$null,$null,$null)
        Monitor-Jobs -CollectionName $CollectionName -MonitorMode -SMITH -SmithScript ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$null,$ProcessLiveSearchServiceInfo,$null,$null,$null) -SmithFlag 'RetrieveFile' -InputValues $InputValues
    }


    if ($ProcessLiveSearchNetworkConnectionsCheckbox) {
        Invoke-Command -ScriptBlock ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$null,$null,$ProcessLiveSearchNetworkConnections,$null,$null)
        Monitor-Jobs -CollectionName $CollectionName -MonitorMode -SMITH -SmithScript ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$null,$null,$ProcessLiveSearchNetworkConnections,$null,$null) -SmithFlag 'RetrieveFile' -InputValues $InputValues
    }
    

    if ($ProcessLiveSearchHashesSignerCertsCheckbox) {
        Invoke-Command -ScriptBlock ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$null,$null,$null,$ProcessLiveSearchHashesSignerCerts,$null)
        Monitor-Jobs -CollectionName $CollectionName -MonitorMode -SMITH -SmithScript ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$null,$null,$null,$ProcessLiveSearchHashesSignerCerts,$null) -SmithFlag 'RetrieveFile' -InputValues $InputValues
    }


    if ($ProcessLiveSearchCompanyProductCheckbox) {
        Invoke-Command -ScriptBlock ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$null,$null,$null,$null,$ProcessLiveSearchCompanyProduct)
        Monitor-Jobs -CollectionName $CollectionName -MonitorMode -SMITH -SmithScript ${function:MonitorJobScriptBlock} -ArgumentList @($CollectionName,$ProcessesLiveRegex,$null,$null,$null,$null,$null,$null,$null,$ProcessLiveSearchCompanyProduct) -SmithFlag 'RetrieveFile' -InputValues $InputValues
    }
