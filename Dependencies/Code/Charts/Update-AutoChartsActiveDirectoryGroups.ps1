...
function Update-AutoChartsActiveDirectoryGroups {
    param($ServerToQuery)
        ...
        #>
            if ($script:ComputerListProvideCredentialsCheckBox.Checked) {
                ...
                if ($AutoChartProtocolWinRMRadioButton.checked) {
                    ...
                    Invoke-Command -ScriptBlock { Get-ADGroup -Filter * -Properties * | Select-Object Name, GroupScope, Created, Modified, Members, Memberof, @{n='MembersCount';e={$_.Members.count}}, @{n='MemberOfCount';e={$_.MemberOf.count}} } `
                    -ComputerName $ServerToQuery `
                    -AsJob -JobName "PoSh-EasyWin: $($CollectionName) -- $($ServerToQuery)" `
                    -Credential $script:Credential
                }
                ...
                elseif ($AutoChartProtocolRPCRadioButton.checked) {
                    $CollectionName = 'Active Directory Groups - (RPC)'
                ...
                elseif ($AutoChartProtocolSMBRadioButton.checked) {
                    $CollectionName = 'Active Directory Groups - (SMB)'
...
Elasped Time:  $($Timecount -replace '-','')"
                    ...
                    & $PsExecPath "\\$ServerToQuery" -AcceptEULA -NoBanner -u $UserName -p $Password powershell -command "Invoke-Command -ScriptBlock { Get-ADGroup -Filter * -Properties * | Select-Object Name, GroupScope, Created, Modified, Members, Memberof, @{n='MembersCount';e={$_.Members.count}}, @{n='MemberOfCount';e={$_.MemberOf.count}} } | ConvertTo-Csv -NoType" | ConvertFrom-Csv | Export-CSV "$($script:CollectionSavedDirectoryTextBox.text)\Results By Endpoints\$CollectionName\$CollectionName -- $ServerToQuery.csv" -NoTypeInformation
