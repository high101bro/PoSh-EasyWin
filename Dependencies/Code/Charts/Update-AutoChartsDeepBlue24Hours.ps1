...
function Update-AutoChartsDeepBlue24Hours {
        ...
        #>
            if ($ComputerListProvideCredentialsCheckBox.Checked) {
                ...
                if ($AutoChartProtocolWinRMRadioButton.checked) {
                    ...
                    Invoke-Command -FilePath "$Dependencies\Code\Main Body\Invoke-DeepBlue24Hours.ps1" `
...
Elasped Time:  $($Timecount -replace '-','')"
                    ...
                    & $PsExecPath "\\$TargetComputer" -AcceptEULA -NoBanner -u $UserName -p $Password powershell -command "Invoke-Command -FilePath '$Dependencies\Code\Main Body\Invoke-DeepBlue24Hours.ps1' -ArgumentList @($RegexFile,$WhiteListFile)" | ConvertTo-Csv -NoType | ConvertFrom-Csv | Export-CSV "$($script:CollectionSavedDirectoryTextBox.text)\Results By Endpoints\$CollectionName\$CollectionName -- $TargetComputer.csv" -NoTypeInformation
...
Elasped Time:  $($Timecount -replace '-','')"
                    ...
                    & $PsExecPath "\\$TargetComputer" -AcceptEULA -NoBanner powershell -command "Invoke-Command -FilePath '$Dependencies\Code\Main Body\Invoke-DeepBlue24Hours.ps1' -ArgumentList @($RegexFile,$WhiteListFile)" | ConvertTo-Csv -NoType | ConvertFrom-Csv | Export-CSV "$($script:CollectionSavedDirectoryTextBox.text)\Results By Endpoints\$CollectionName\$CollectionName -- $TargetComputer.csv" -NoTypeInformation
                    $script:ProgressBarFormProgressBar.Value += 1
                }
            }
        #}
