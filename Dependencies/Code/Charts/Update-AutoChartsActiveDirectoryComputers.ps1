...
function Update-AutoChartsActiveDirectoryComputers {
    param($ServerToQuery)
    ...
    $ExecutionStartTime = Get-Date
        ...
        #>
            if ($ComputerListProvideCredentialsCheckBox.Checked) {
                if (!$script:Credential) { Create-NewCredentials }
                if ($AutoChartProtocolWinRMRadioButton.checked) {
                    $CollectionName = 'Active Directory Computers - (WinRM)'

                    #Invoke-Command -ScriptBlock { Get-ADComputer -Filter * -Properties OperatingSystem, OperatingSystemVersion, OperatingSystemServicePack, OperatingSystemHotfix, IPv4Address, Enabled, Created, Modified, LastLogonDate, PasswordLastSet | Select-Object OperatingSystem, OperatingSystemVersion, OperatingSystemServicePack, OperatingSystemHotfix, IPv4Address, Enabled, @{n='Created';e={($_.Created).ToString('yyyy-MM-dd')}}, @{n='Modified';e={($_.Modified).ToString('yyyy-MM-dd')}}, @{n='LastLogonDate';e={($_.LastLogonDate).ToString('yyyy-MM-dd')}}, @{n='PasswordLastSet';e={($_.PasswordLastSet).ToString('yyyy-MM-dd')}} } `
                    Invoke-Command -ScriptBlock { Get-ADComputer -Filter * -Properties OperatingSystem, OperatingSystemVersion, OperatingSystemServicePack, OperatingSystemHotfix, IPv4Address, Enabled, Created, Modified, LastLogonDate, PasswordLastSet } `
                    -ComputerName $ServerToQuery `
                    -AsJob -JobName "PoSh-EasyWin: $($CollectionName) -- $($ServerToQuery)" `
                    -Credential $script:Credential
                }
                ...
                elseif ($AutoChartProtocolRPCRadioButton.checked) {
                    $CollectionName = 'Active Directory Computers - (RPC)'
                ...
                elseif ($AutoChartProtocolSMBRadioButton.checked) {
                    $CollectionName = 'Active Directory Computers - (SMB)'
...
Elasped Time:  $($Timecount -replace '-','')"
                    ...
                    & $PsExecPath "\\$ServerToQuery" -AcceptEULA -NoBanner -u $UserName -p $Password powershell -command "Invoke-Command -ScriptBlock { Get-ADComputer -Filter * -Properties OperatingSystem, OperatingSystemVersion, OperatingSystemServicePack, OperatingSystemHotfix, IPv4Address, Enabled, Created, Modified, LastLogonDate, PasswordLastSet } | ConvertTo-Csv -NoType" | ConvertFrom-Csv | Export-CSV "$($script:CollectionSavedDirectoryTextBox.text)\Results By Endpoints\$CollectionName\$CollectionName -- $ServerToQuery.csv" -NoTypeInformation
...
Elasped Time:  $($Timecount -replace '-','')"
                    ...
                    & $PsExecPath "\\$ServerToQuery" -AcceptEULA -NoBanner powershell -command "Invoke-Command -ScriptBlock { Get-ADComputer -Filter * -Properties OperatingSystem, OperatingSystemVersion, OperatingSystemServicePack, OperatingSystemHotfix, IPv4Address, Enabled, Created, Modified, LastLogonDate, PasswordLastSet } | ConvertTo-Csv -NoType" | ConvertFrom-Csv | Export-CSV "$($script:CollectionSavedDirectoryTextBox.text)\$CollectionName\$CollectionName -- $ServerToQuery.csv" -NoTypeInformation
                    $script:ProgressBarFormProgressBar.Value += 1
                }
            }
        #}
