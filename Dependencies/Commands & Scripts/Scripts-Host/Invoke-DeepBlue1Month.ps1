...
function Invoke-DeepBlueAll {
    ...
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddMonths(-1);EndTime=(Get-Date);Logname='Security';ID=@(4688,4672,4720,4728,4732,4756,4625,4673,4674,4648,1102)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddMonths(-1);EndTime=(Get-Date);Logname='System';ID=@(104,7030,7036,7040,7045)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddMonths(-1);EndTime=(Get-Date);Logname='Application';ID=@(2)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddMonths(-1);EndTime=(Get-Date);logname='Microsoft-Windows-AppLocker/EXE and DLL';ID=@(8003,8004,8006,8007)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddMonths(-1);EndTime=(Get-Date);logname='Microsoft-Windows-PowerShell/Operational';ID=@(4103,4104)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddMonths(-1);EndTime=(Get-Date);logname='Windows PowerShell';ID=800} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddMonths(-1);EndTime=(Get-Date);logname='Microsoft-Windows-Sysmon/Operational';ID=@(1,7)} -ErrorAction SilentlyContinue
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQULAfYhEJZEJ/ZKuj9ug5nqnv2
# rP6gggM6MIIDNjCCAh6gAwIBAgIQVnYuiASKXo9Gly5kJ70InDANBgkqhkiG9w0B
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUqL1o2Ww4J2eIZ6EFGOAOYIqOs0cwDQYJKoZI
# hvcNAQEBBQAEggEAo7MNtYuTLkRpr0fGUQylZIEhUde50XGj6+wSFeDUv28WtCmS
# /URPX7WoOAWf4xmEIeQRTeixoT21IcD3N8CPiPz7OrjiIrsWXQzgsvhmRMIHeVbN
# cs30jiHknfr9Py7gkWGl2qYpe51NIm1PaUKlL9dFt5Wh65YYfwhyN54GiEiDY8CG
# gKuv1Vx7yakZKfGsx2n3xeB9Nt05Ef7d16mt/LN0Wmve4H+q1VH2e4r36kAR+IdP
# uTXFDYPhXJrlZuD0pZnxeR+tFP3G3kPXBhHCSBakAy44jRv61UreiZJPdPB7WLw4
# tjaBAkRPL2STBPpJ0veXpMa1iqtjw+Shgv0Sfw==
