...
function Invoke-DeepBlueAll {
    ...
    $events += Get-WinEvent -filterhashtable @{Logname='Security';ID=@(4688,4672,4720,4728,4732,4756,4625,4673,4674,4648,1102)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{Logname='System';ID=@(104,7030,7036,7040,7045)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{Logname='Application';ID=@(2)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{logname='Microsoft-Windows-AppLocker/EXE and DLL';ID=@(8003,8004,8006,8007)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{logname='Microsoft-Windows-PowerShell/Operational';ID=@(4103,4104)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{logname='Windows PowerShell';ID=800} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{logname='Microsoft-Windows-Sysmon/Operational';ID=@(1,7)} -ErrorAction SilentlyContinue
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUGTTD2N8rr85Z/zto2ojxcMP9
# C62gggM6MIIDNjCCAh6gAwIBAgIQeugH5LewQKBKT6dPXhQ7sDANBgkqhkiG9w0B
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQUgboAC52RMZBFQ7uwZCww7riytDYwDQYJKoZI
# hvcNAQEBBQAEggEAUG2u+tHsUvJ7hYJtkIQ7ShquGcCS/PcS1VDFRqTdRb9BgEVM
# wAwQtIAamhKHZKChp0diTGtScswDQsquxc2EHpj6mYrZMlrvrwdLSRsrjBrhWCLU
# sJxtj5TeXt3U1edGnsF+dTus/Io0l279xb5qG/0MWzTu2ZQALO6zBtd0WLbJ2VGt
# WCMqagizZcQSSBwuhRGj0I8RnA2s00TU5kl57e26beTFNPaXdTevYTbFiX1uwN3Z
# 5VSRyS7KsKHnnOShhGGt83WbptKzwyPcZq5jWHptWP3eaxVb18b73vWt6GSAlYDY
# 9n0FesVVjdwq0yO8rrxDUf5NGLVWrP2dFThPOg==
