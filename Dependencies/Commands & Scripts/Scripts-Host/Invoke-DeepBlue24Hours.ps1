...
###############################################################
param (
    $RegexFile     = $(Get-Content './Invoke-DeepBlue-regexes.txt'),
    $WhiteListFile = $(Get-Content './Invoke-DeepBlue-whitelist.txt')
)
...
function Invoke-DeepBlueAll {
    ...
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);Logname='Security';ID=@(4688,4672,4720,4728,4732,4756,4625,4673,4674,4648,1102)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);Logname='System';ID=@(104,7030,7036,7040,7045)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);Logname='Application';ID=@(2)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);logname='Microsoft-Windows-AppLocker/EXE and DLL';ID=@(8003,8004,8006,8007)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);logname='Microsoft-Windows-PowerShell/Operational';ID=@(4103,4104)} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);logname='Windows PowerShell';ID=800} -ErrorAction SilentlyContinue
    $events += Get-WinEvent -filterhashtable @{StartTime=(Get-Date).AddHours(-24);EndTime=(Get-Date);logname='Microsoft-Windows-Sysmon/Operational';ID=@(1,7)} -ErrorAction SilentlyContinue
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUBSc8Cq6NZQqXvkboZbhva43X
# oSigggM6MIIDNjCCAh6gAwIBAgIQVnYuiASKXo9Gly5kJ70InDANBgkqhkiG9w0B
...
# NwIBFTAjBgkqhkiG9w0BCQQxFgQU08ZGLLP2NeEZ2MIEPP7xA7acuB0wDQYJKoZI
# hvcNAQEBBQAEggEAb1pq4miYoU9rpZHN2JzB+oOuvRGD78wOD21s+T2tqQwQq4uT
# wBVgTANOPznnz17aph7SjAXzzw6291Z+4nxvQ75hs3kHIgunpNslZ6rRH8hcroF5
# NK0+kxOYSlt3vVbopco9hKmwZBO4nt9/f7VlPrzJFOH4MjWp8c2Nw3xvGVwTNlsw
# e/0Wf0OKr4JjBbrEt/KP+Gu/2y0Xyb6BIO8bDFBM+W63jjhh9oe/Nq/rikhsg1sA
# Y2ipCfPdSyPWv2TXm6vgrCvODQMqASDWqsIK9iGv7GGsi8tCX8Dnj58kSmQ0PssB
# nIk1rDD1OMenwBoKvwMgBrlulMGSGw2I29UOfQ==
